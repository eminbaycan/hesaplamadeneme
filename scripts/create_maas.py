...
export default function MaasHesaplama() {
  ...
  const hesapla = () => {
    ...
    // Basitleştirilmiş güncel oranlar (Eğitim/Araç amaçlıdır, asgari ücret istisnası hariç tutulmuştur)
    const sgkOrani = 0.14;
    const issizlikOrani = 0.01;
    const gelirVergisiOrani = 0.15; // 1. Dilim baz alınmıştır
    ...
    if (hesapTuru === 'brutten-nete') {
      ...
      // Net = Brüt - (Brüt*0.14) - (Brüt*0.01) - ((Brüt - Brüt*0.15)*0.15) - (Brüt*0.00759)
      // Net = Brüt * (1 - 0.14 - 0.01 - 0.1275 - 0.00759)
      // Net = Brüt * 0.71491
      brut = Number(tutar) / 0.71491; 
    }
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">Maaş Hesaplama (Brüt ➔ Net)</h1>
...
"""
with open("src/tools/finans/MaasHesaplama.tsx", "w") as f: f.write(content)
