...
template_yks = """import React, { useState } from 'react';
...
export default function YksHesaplama() {
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">YKS (TYT) Puan Hesaplama</h1>
...
with open("src/tools/egitim/YksPuanHesaplama.tsx", "w") as f: f.write(template_yks)

template_kpss = """import React, { useState } from 'react';
import { ChevronRight, RefreshCw, Copy, Info, AlertCircle } from 'lucide-react';
import { Link } from 'react-router-dom';
import { ToolIcon } from '../../components/icons/ToolIcon';
import { AdSlot } from '../../components/ads/AdSlot';
...
export default function KpssPuanHesaplama() {
  const [genelYetenek, setGenelYetenek] = useState<number | ''>('');
  const [genelKultur, setGenelKultur] = useState<number | ''>('');
  ...
  const hesapla = () => {
    if (genelYetenek === '' || genelKultur === '') {
      setError("*Lütfen tüm netleri doldurun.");
      setSonuc(null);
      return;
    }
    
    // KPSS Lisans P3 Puanı = Temel Puan + (GY Net x Katsayı) + (GK Net x Katsayı)
    // Katsayılar her yıl değişmekle beraber GY için ~0.5, GK için ~0.5 ve taban puan ~40'tır
    const p3 = 40 + (Number(genelYetenek) * 0.5) + (Number(genelKultur) * 0.5);
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">KPSS Puan Hesaplama (P3 - Lisans)</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          Genel Yetenek ve Genel Kültür netlerinizi girerek KPSS P3 lisans yerleştirme puanınızı hesaplayın.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-4">
          <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm">
            ...
            <div className="space-y-5">
              <div>
                <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">
                  Genel Yetenek Neti <span className="text-red-500">*</span>
                </label>
                <input 
                  type="number" 
                  value={genelYetenek}
                  onChange={(e) => {
                    setGenelYetenek(e.target.value === '' ? '' : Number(e.target.value));
                    setError(null);
                  }}
                  placeholder="Örn: 45" 
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-black/10 dark:border-white/10 rounded-xl px-4 py-3 text-slate-700 dark:text-white font-medium focus:bg-white dark:focus:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-[#0056b3]/20 dark:focus:ring-blue-500/30 focus:border-[#0056b3] dark:focus:border-blue-500 transition-all"
              ...
              <div>
                <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">
                  Genel Kültür Neti <span className="text-red-500">*</span>
                </label>
                <input 
                  type="number" 
                  value={genelKultur}
                  onChange={(e) => {
                    setGenelKultur(e.target.value === '' ? '' : Number(e.target.value));
                    setError(null);
                  }}
                  placeholder="Örn: 40" 
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-black/10 dark:border-white/10 rounded-xl px-4 py-3 text-slate-700 dark:text-white font-medium focus:bg-white dark:focus:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-[#0056b3]/20 dark:focus:ring-blue-500/30 focus:border-[#0056b3] dark:focus:border-blue-500 transition-all"
          ...
          <div className="lg:col-span-5 flex flex-col">
            <div className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-3xl p-6 md:p-8 flex-1 relative overflow-hidden shadow-lg border border-black/5 dark:border-slate-800">
              ...
              <div className="relative z-10 flex flex-col h-full">
                <div className="flex items-center justify-between mb-8 opacity-90">
                  <h3 className="text-sm font-bold uppercase tracking-widest text-slate-800 dark:text-white">P3 Puanınız</h3>
                </div>
                ...
                <div className="flex-1 flex flex-col justify-center gap-6">
                  {sonuc !== null ? (
                    <>
                      <div>
                        <p className="text-slate-500 dark:text-slate-400 text-xs mb-1 font-bold uppercase">KPSS P3 (Lisans)</p>
                        <div className="text-4xl font-black tracking-tighter text-[#0056b3] dark:text-blue-400">
                          {sonuc.p3Puan.toLocaleString('tr-TR', { maximumFractionDigits: 3 })}
                        </div>
                      </div>
                    </>
                  ) : (
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm">
        <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-6 flex items-center gap-2 border-b border-black/5 dark:border-white/10 pb-4">
          <Info className="text-[#0056b3] dark:text-blue-400 shrink-0" size={24} /> 
          KPSS Puan Hesaplama Hakkında Her Şey ve Sıkça Sorulan Sorular
        </h3>
        ...
        <div className="prose prose-sm md:prose-base max-w-none text-slate-600 dark:text-slate-400 space-y-8 leading-relaxed">
          <section>
            <h4 className="text-[17px] font-bold text-slate-800 dark:text-slate-200 mb-3">Nasıl Kullanılır?</h4>
            <p>
              KPSS Lisans (B Grubu Kadrolar) sınavında yapmış olduğunuz Genel Yetenek (Türkçe, Matematik) ve Genel Kültür (Tarih, Coğrafya, Vatandaşlık) netlerinizi ilgili alanlara girin. Araç size tahmini P3 puanınızı hesaplayacaktır.
            </p>
          </section>

          <section>
            <h4 className="text-[17px] font-bold text-slate-800 dark:text-slate-200 mb-3">KPSS P3 Puanı Nedir?</h4>
            <p>
              KPSS P3 Puan Türü, B Grubu Kadrolara (Mühendislik, Mimarlık, Memurluk, vs.) yapılacak atamalarda kullanılan ve sadece Genel Yetenek ile Genel Kültür testlerinin ağırlıklandırıldığı puan türüdür.
            </p>
          </section>
          ...
          <div className="pt-2">
            <div className="space-y-4 bg-slate-50 dark:bg-slate-800/30 p-4 md:p-6 rounded-2xl border border-black/5 dark:border-white/5">
              <div>
                <div className="font-semibold text-slate-800 dark:text-slate-200 mb-1">Kullanılan Formüller:</div>
                <div className="bg-white dark:bg-slate-900 border border-black/10 dark:border-white/10 px-4 py-2.5 rounded-xl font-mono text-sm text-[#0056b3] dark:text-blue-400 mb-2 overflow-x-auto">
                  Tahmini Puan = Taban Puan + (GY Net x Katsayı) + (GK Net x Katsayı)
                </div>
                <p className="text-xs text-slate-500 mt-2">Not: Her yıl sınava girenlerin sayısına ve sınav zorluk derecesine göre standart sapma değiştiği için bu hesaplama ortalama değerlere (0.5 katsayı) dayanan bir tahmindir.</p>
              </div>
            </div>
          </div>
          ...
          <section className="pt-4 border-t border-black/5 dark:border-white/5">
            ...
            <div className="space-y-6">
              <div>
                <h5 className="font-bold text-slate-800 dark:text-slate-200 mb-2">Eğitim Bilimleri netlerini buraya girecek miyim?</h5>
                <p>Hayır, öğretmen adayları için P10 ve P121 puan türleri kullanılır. Bu araç sadece memurluk atamalarında kullanılan P3 (Lisans B Grubu) puanını hesaplamaktadır.</p>
              </div>
            </div>
          </section>
        </div>
...
with open("src/tools/egitim/KpssPuanHesaplama.tsx", "w") as f: f.write(template_kpss)
