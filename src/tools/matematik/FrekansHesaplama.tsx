...
export default function FrekansHesaplama() {
  const [val1, setVal1] = useState<string>('');
  const [val2, setVal2] = useState<string>('343');
  ...
  const hesapla = () => {
    ...
    setError(null);
      ...
      if (t <= 0 || isNaN(t)) {
        setError('*Periyot değeri sıfırdan büyük olmalıdır.');
        setSonuc(null);
        return;
      }
      const f = 1 / t;
      setSonuc(`${f.toLocaleString('tr-TR', { maximumFractionDigits: 4 })} Hz`);
      setDetay(`f = 1 / ${t} sn = ${f.toFixed(2)} Hz (${(f / 1000).toFixed(4)} kHz)`);
  };
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      ...
      <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mb-6 font-medium">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-800 dark:text-slate-300">Frekans Hesaplama</span>
      </div>

      {/* 2. BAŞLIK VE AÇIKLAMA */}
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">Frekans Hesaplama 2026 - Ücretsiz ve Hızlı Sonuçlar</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          Dalga boyu (λ), periyot (T) veya ışık hızı (c) parametrelerinden faydalanarak frekans (Hz, kHz, MHz) değerini hesaplayın.
        </p>
      </div>
          ...
          <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">Periyot T (saniye)</label>
                  <input
                    type="number"
                    value={val1}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">Dalga Hızı v (m/s - Opsiyonel)</label>
                  <input
                    type="number"
                    value={val2}
                    onChange={(e) => { setVal2(e.target.value); setError(null); }}
                    placeholder="343 (Ses Hızı)"
                    className="w-full bg-slate-50 dark:bg-slate-800 border border-black/10 dark:border-white/10 rounded-xl px-4 py-3 text-slate-700 dark:text-white font-medium focus:bg-white dark:focus:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-[#0056b3]/20 dark:focus:ring-blue-500/30 transition-all"
                  />
                </div>
              </div>
                  ...
                  {sonuc !== null && (
                    <div className="mt-4 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-black/5 dark:border-white/5">
                      ...
                      <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
                        Birim zamanda (saniyede) gerçekleşen periyodik dalga veya titreşim sayısıdır.
                      </p>
                    </div>
                  )}
                </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 mb-8 space-y-6 text-slate-700 dark:text-slate-300 leading-relaxed shadow-sm">
        ...
        <div>
          <h4 className="text-base font-bold text-slate-900 dark:text-white mb-2">Frekans Hesaplama Nedir ve Nasıl Hesaplanır?</h4>
          <p className="text-sm">
            Frekans Hesaplama, matematik ve geometri alanında sıkça başvurulan temel hesaplama disiplinlerinden biridir. Dalga boyu (λ), periyot (T) veya ışık hızı (c) parametrelerinden faydalanarak frekans (Hz, kHz, MHz) değerini hesaplayın. Hesaplama aracımız girilen tüm sayısal parametreleri analiz ederek kurallara uygun biçimde sonuçlandırır.
          </p>
        </div>
        ...
        <div>
          ...
          <div className="space-y-3 text-sm">
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">Frekansın birimi nedir?</h5>
              <p>Uluslararası SI birim sisteminde frekans birimi Hertz (Hz) olup saniyedeki titreşim sayısını ifade eder.</p>
            </div>
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">Periyot ile frekans arasındaki ilişki nedir?</h5>
              <p>Periyot ve frekans birbirinin çarpmaya göre tersidir (f = 1 / T).</p>
            </div>
          </div>
        </div>
      </div>

            <div className="mt-8"><RelatedTools currentToolId="frekans-hesaplama" category="matematik" /></div>
<Disclaimer />
    </div>
  );
}
