...
export default function KesirHesaplama() {
  ...
  const [val3, setVal3] = useState<string>('1');
  ...
  const hesapla = () => {
    ...
    setError(null);
      ...
      const p1 = parseFloat(val1) || 0;
      const d1 = parseFloat(val2) || 1;
      const p2 = parseFloat(val3) || 0;
      const d2Elem = document.getElementById('d2_input') as HTMLInputElement;
      const d2 = d2Elem ? parseFloat(d2Elem.value) || 1 : 1;
      if (d1 === 0 || d2 === 0) {
        setError('*Payda sıfır olamaz.');
        setSonuc(null);
        return;
      }
      const top = (p1 * d2) + (p2 * d1);
      const bot = d1 * d2;
      setSonuc(`${top}/${bot} (${(top/bot).toFixed(2)})`);
      setDetay(`Toplama: (${p1}×${d2} + ${p2}×${d1}) / (${d1}×${d2}) = ${top}/${bot}`);
  };
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      ...
      <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mb-6 font-medium">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-800 dark:text-slate-300">Kesir Hesaplama</span>
      </div>

      {/* 2. BAŞLIK VE AÇIKLAMA */}
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">Kesir Hesaplama 2026 - Ücretsiz ve Hızlı Sonuçlar</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          İki kesir arasında toplama, çıkarma, çarpma, bölme ve sadeleştirme işlemlerini adım adım çözün.
        </p>
      </div>
      ...
      <div className="calculator-container mb-8 relative border border-slate-200 dark:border-slate-700 rounded-2xl p-5 sm:p-8 mt-8">
        <div className="absolute -top-3.5 left-4 sm:left-8 bg-[#eef2f7] dark:bg-slate-950 px-4">
          <h2 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <ToolIcon name="bolme" size={20} className="text-[#0056b3] dark:text-blue-400" />
            Kesir Hesaplama
          </h2>
        </div>
          ...
          <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">1. Pay</label>
                  <input type="number" value={val1} onChange={(e) => { setVal1(e.target.value); setError(null); }} className="w-full bg-slate-50 dark:bg-slate-800 border border-black/10 dark:border-white/10 rounded-xl px-4 py-3 text-slate-700 dark:text-white text-sm" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">1. Payda</label>
                  <input type="number" value={val2} onChange={(e) => { setVal2(e.target.value); setError(null); }} className="w-full bg-slate-50 dark:bg-slate-800 border border-black/10 dark:border-white/10 rounded-xl px-4 py-3 text-slate-700 dark:text-white text-sm" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">2. Pay</label>
                  <input type="number" value={val3} onChange={(e) => { setVal3(e.target.value); setError(null); }} className="w-full bg-slate-50 dark:bg-slate-800 border border-black/10 dark:border-white/10 rounded-xl px-4 py-3 text-slate-700 dark:text-white text-sm" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">2. Payda</label>
                  <input type="number" defaultValue="2" id="d2_input" className="w-full bg-slate-50 dark:bg-slate-800 border border-black/10 dark:border-white/10 rounded-xl px-4 py-3 text-slate-700 dark:text-white text-sm" />
                </div>
              </div>
                  ...
                  {sonuc !== null && (
                    <div className="mt-4 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-black/5 dark:border-white/5">
                      ...
                      <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
                        Pay ve paydaları verilen iki rasyonel sayının birleşik aritmetik işlem sonucudur.
                      </p>
                    </div>
                  )}
                </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 mb-8 space-y-6 text-slate-700 dark:text-slate-300 leading-relaxed shadow-sm">
        ...
        <div>
          <h4 className="text-base font-bold text-slate-900 dark:text-white mb-2">Kesir Hesaplama Nedir ve Nasıl Hesaplanır?</h4>
          <p className="text-sm">
            Kesir Hesaplama, matematik ve geometri alanında sıkça başvurulan temel hesaplama disiplinlerinden biridir. İki kesir arasında toplama, çıkarma, çarpma, bölme ve sadeleştirme işlemlerini adım adım çözün. Hesaplama aracımız girilen tüm sayısal parametreleri analiz ederek kurallara uygun biçimde sonuçlandırır.
          </p>
        </div>
        ...
        <div>
          ...
          <div className="space-y-3 text-sm">
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">Kesirlerde payda eşitliği neden gereklidir?</h5>
              <p>Toplama ve çıkarma işlemlerinde aynı büyüklükteki birim parçaların toplanabilmesi için paydalar eşitlenir.</p>
            </div>
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">Bölme işlemi nasıl yapılır?</h5>
              <p>Birinci kesir aynen kalır, ikinci kesir ters çevrilerek birinci ile çarpılır.</p>
            </div>
          </div>
        </div>
      </div>

            <div className="mt-8"><RelatedTools currentToolId="kesir-hesaplama" category="matematik" /></div>
<Disclaimer />
    </div>
  );
}
