...
export default function LogaritmaHesaplama() {
  ...
  const hesapla = () => {
    ...
    setError(null);
      ...
      if (x <= 0 || b <= 0 || b === 1) {
        setError('*Sayı ve taban sıfırdan büyük, taban 1\'den farklı olmalıdır.');
        setSonuc(null);
        return;
      }
      const res = Math.log(x) / Math.log(b);
      setSonuc(res.toFixed(4));
      setDetay(`log_${b}(${x}) = ${res.toFixed(6)} | Doğal Logaritma ln(${x}) = ${Math.log(x).toFixed(4)}`);
  };
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      ...
      <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mb-6 font-medium">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-800 dark:text-slate-300">Logaritma Hesaplama</span>
      </div>

      {/* 2. BAŞLIK VE AÇIKLAMA */}
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">Logaritma Hesaplama 2026 - Ücretsiz ve Hızlı Sonuçlar</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          Doğal logaritma (ln), 10 tabanında logaritma (log10) veya herhangi bir özel tabanda logaritmik değerleri hesaplayın.
        </p>
      </div>
          ...
          <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">Sayı (x &gt; 0)</label>
                  <input
                    type="number"
                    value={val1}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">Taban (b &gt; 0, b ≠ 1)</label>
                  <input
                    type="number"
                    value={val2}
                  ...
                  {sonuc !== null && (
                    <div className="mt-4 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-black/5 dark:border-white/5">
                      ...
                      <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
                        Girdiğiniz tabanın hangi üssünün (kuvvetinin) girdiğiniz sayıya eşit olduğunu gösteren üs değeridir.
                      </p>
                    </div>
                  )}
                </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 mb-8 space-y-6 text-slate-700 dark:text-slate-300 leading-relaxed shadow-sm">
        ...
        <div>
          <h4 className="text-base font-bold text-slate-900 dark:text-white mb-2">Logaritma Hesaplama Nedir ve Nasıl Hesaplanır?</h4>
          <p className="text-sm">
            Logaritma Hesaplama, matematik ve geometri alanında sıkça başvurulan temel hesaplama disiplinlerinden biridir. Doğal logaritma (ln), 10 tabanında logaritma (log10) veya herhangi bir özel tabanda logaritmik değerleri hesaplayın. Hesaplama aracımız girilen tüm sayısal parametreleri analiz ederek kurallara uygun biçimde sonuçlandırır.
          </p>
        </div>
        ...
        <div>
          ...
          <div className="space-y-3 text-sm">
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">Doğal logaritma (ln) nedir?</h5>
              <p>Tabanı Euler sayısı (e ≈ 2.71828) olan logaritma fonksiyonudur.</p>
            </div>
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">Logaritma tabanı neden 1 olamaz?</h5>
              <p>1 sayısının tüm pozitif kuvvetleri yine 1 olduğundan 1 tabanlı logaritma tanımsızdır.</p>
            </div>
          </div>
        </div>
      </div>

            <div className="mt-8"><RelatedTools currentToolId="logaritma-hesaplama" category="matematik" /></div>
<Disclaimer />
    </div>
  );
}
