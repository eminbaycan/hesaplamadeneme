...
export default function IkinciDerecedenDenklemHesaplama() {
  const [val1, setVal1] = useState<string>('');
  const [val2, setVal2] = useState<string>('-5');
  const [val3, setVal3] = useState<string>('6');
  ...
  const hesapla = () => {
    ...
    setError(null);
      ...
      if (isNaN(a) || a === 0) {
        setError('*a katsayısı 0 olamaz (ikinci derece denklem kuralı).');
        setSonuc(null);
        return;
      }
      const delta = b * b - 4 * a * c;
      if (delta < 0) {
        setSonuc('Reel Kök Yok (Δ < 0)');
        setDetay(`Diskriminant Δ = ${delta}. Karmaşık sayılar kümesinde iki kök mevcuttur.`);
      } else {
        const x1 = (-b + Math.sqrt(delta)) / (2 * a);
        const x2 = (-b - Math.sqrt(delta)) / (2 * a);
        setSonuc(`x₁ = ${x1.toFixed(2)}, x₂ = ${x2.toFixed(2)}`);
        setDetay(`Δ = ${delta} | İki farklı reel kök: x₁ = ${x1.toFixed(4)}, x₂ = ${x2.toFixed(4)}`);
      }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      ...
      <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mb-6 font-medium">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-800 dark:text-slate-300">İkinci Dereceden Denklem Hesaplama</span>
      </div>

      {/* 2. BAŞLIK VE AÇIKLAMA */}
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">İkinci Dereceden Denklem Hesaplama 2026 - Ücretsiz ve Hızlı Sonuçlar</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          ax² + bx + c = 0 formundaki ikinci dereceden denklemlerin diskriminant (Δ) ve reel/karmaşık köklerini bulun.
        </p>
      </div>
          ...
          <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">a (x² katsayısı)</label>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">b (x katsayısı)</label>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">c (sabit sayı)</label>
                  ...
                  {sonuc !== null && (
                    <div className="mt-4 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-black/5 dark:border-white/5">
                      ...
                      <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
                        ax² + bx + c = 0 polinom denklemini sıfıra eşitleyen x çözüm kümesidir.
                      </p>
                    </div>
                  )}
                </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 mb-8 space-y-6 text-slate-700 dark:text-slate-300 leading-relaxed shadow-sm">
        ...
        <div>
          <h4 className="text-base font-bold text-slate-900 dark:text-white mb-2">İkinci Dereceden Denklem Hesaplama Nedir ve Nasıl Hesaplanır?</h4>
          <p className="text-sm">
            İkinci Dereceden Denklem Hesaplama, matematik ve geometri alanında sıkça başvurulan temel hesaplama disiplinlerinden biridir. ax² + bx + c = 0 formundaki ikinci dereceden denklemlerin diskriminant (Δ) ve reel/karmaşık köklerini bulun. Hesaplama aracımız girilen tüm sayısal parametreleri analiz ederek kurallara uygun biçimde sonuçlandırır.
          </p>
        </div>
        ...
        <div>
          ...
          <div className="space-y-3 text-sm">
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">Diskriminant (Delta) sıfır olursa ne olur?</h5>
              <p>Denklemin birbirine eşit (çakışık) iki reel kökü olur: x₁ = x₂ = -b / 2a.</p>
            </div>
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">a katsayısı 0 olabilir mi?</h5>
              <p>a = 0 olduğunda denklem 2. dereceden olmaktan çıkar, 1. dereceden lineer denkleme dönüşür.</p>
            </div>
          </div>
        </div>
      </div>

            <div className="mt-8"><RelatedTools currentToolId="ikinci-dereceden-denklem-hesaplama" category="matematik" /></div>
<Disclaimer />
    </div>
  );
}
