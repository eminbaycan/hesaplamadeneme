...
export default function DikdortgenKareUcgenCevreHesaplama() {
  const [val1, setVal1] = useState<string>('');
  const [val2, setVal2] = useState<string>('5');
  const [val3, setVal3] = useState<string>('dikdortgen');
  ...
  const hesapla = () => {
    ...
    setError(null);
      ...
      if (a <= 0) {
        setError('*Lütfen 0\'dan büyük bir kenar uzunluğu girin.');
        setSonuc(null);
        return;
      }
      if (val3 === 'kare') {
        const c = 4 * a;
        setSonuc(`${c.toLocaleString('tr-TR')} birim`);
        setDetay(`Kare Çevresi = 4 × ${a} = ${c} birim`);
      } else if (val3 === 'dikdortgen') {
        const c = 2 * (a + b);
        setSonuc(`${c.toLocaleString('tr-TR')} birim`);
        setDetay(`Dikdörtgen Çevresi = 2 × (${a} + ${b}) = ${c} birim`);
      } else {
        const c = a + b + Math.sqrt(a * a + b * b);
        setSonuc(`${c.toFixed(2)} birim`);
        setDetay(`Üçgen Çevresi (Hipotenüs dahil) ≈ ${c.toFixed(2)} birim`);
      }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      ...
      <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mb-6 font-medium">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-800 dark:text-slate-300">Dikdörtgen, Kare ve Üçgen Çevre Hesaplama</span>
      </div>

      {/* 2. BAŞLIK VE AÇIKLAMA */}
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">Dikdörtgen, Kare ve Üçgen Çevre Hesaplama 2026 - Ücretsiz ve Hızlı Sonuçlar</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          Dikdörtgen, kare ve üçgen gibi temel geometrik şekillerin çevre uzunluklarını kenar değerlerini girerek anında hesaplayın.
        </p>
      </div>
      ...
      <div className="calculator-container mb-8 relative border border-slate-200 dark:border-slate-700 rounded-2xl p-5 sm:p-8 mt-8">
        <div className="absolute -top-3.5 left-4 sm:left-8 bg-[#eef2f7] dark:bg-slate-950 px-4">
          <h2 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <ToolIcon name="cevre" size={20} className="text-[#0056b3] dark:text-blue-400" />
            Dikdörtgen, Kare ve Üçgen Çevre Hesaplama
          </h2>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-4">
          ...
          <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm">
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">Şekil Türü</label>
                <select
                  value={val3}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">1. Kenar / a (cm/m)</label>
                  <input
                    type="number"
                    value={val1}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">2. Kenar / b (veya Diğer)</label>
                  <input
                    type="number"
                    value={val2}
                  ...
                  {sonuc !== null && (
                    <div className="mt-4 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-black/5 dark:border-white/5">
                      ...
                      <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
                        Girilen kenar uzunluklarına sahip geometrik şeklin tüm dış sınır çizgilerinin toplam uzunluğudur.
                      </p>
                    </div>
                  )}
                </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 mb-8 space-y-6 text-slate-700 dark:text-slate-300 leading-relaxed shadow-sm">
        ...
        <div>
          <h4 className="text-base font-bold text-slate-900 dark:text-white mb-2">Dikdörtgen, Kare ve Üçgen Çevre Hesaplama Nedir ve Nasıl Hesaplanır?</h4>
          <p className="text-sm">
            Dikdörtgen, Kare ve Üçgen Çevre Hesaplama, matematik ve geometri alanında sıkça başvurulan temel hesaplama disiplinlerinden biridir. Dikdörtgen, kare ve üçgen gibi temel geometrik şekillerin çevre uzunluklarını kenar değerlerini girerek anında hesaplayın. Hesaplama aracımız girilen tüm sayısal parametreleri analiz ederek kurallara uygun biçimde sonuçlandırır.
          </p>
        </div>
        ...
        <div>
          ...
          <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-xl font-mono text-sm border border-slate-200 dark:border-slate-700">
            {"Kare: 4a | Dikdörtgen: 2(a + b) | Üçgen: a + b + c"}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-3 text-sm">
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">Karenin çevresi nasıl hesaplanır?</h5>
              <p>Tüm 4 kenarı eşit olduğundan bir kenar uzunluğu 4 ile çarpılarak bulunur (Ç = 4a).</p>
            </div>
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">Dikdörtgen çevresinde kısa ve uzun kenar ilişkisi nedir?</h5>
              <p>Dikdörtgenin ikişer eşit paralel kenarı olduğundan (kısa kenar + uzun kenar) toplamı 2 ile çarpılır.</p>
            </div>
          </div>
        </div>
      </div>

            <div className="mt-8"><RelatedTools currentToolId="dikdortgen-kare-ucgen-cevre-hesaplama" category="matematik" /></div>
<Disclaimer />
    </div>
  );
}
