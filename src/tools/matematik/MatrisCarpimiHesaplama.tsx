...
export default function MatrisCarpimiHesaplama() {
  const [val1, setVal1] = useState<string>('');
  const [val2, setVal2] = useState<string>('5 6, 7 8');
  ...
  const hesapla = () => {
    ...
    setError(null);
      ...
      try {
        const parseMat = (s: string) => s.split(',').map(row => row.trim().split(/\s+/).map(Number));
        const A = parseMat(val1);
        const B = parseMat(val2);
        if (A.some(r => r.some(isNaN)) || B.some(r => r.some(isNaN))) throw new Error('Geçersiz matris');
        if (A[0].length !== B.length) {
          setError('*A matrisinin sütun sayısı ile B matrisinin satır sayısı eşit olmalıdır.');
          setSonuc(null);
          return;
        }
        const C = A.map(row => 
        ...
        const resStr = JSON.stringify(C).replace(/],/g, '], ');
        setSonuc(resStr);
        setDetay(`Boyut: ${A.length}x${B[0].length} Matris Çarpımı Başarıyla Tamamlandı.`);
      } catch {
        setError('*Lütfen geçerli matris sayıları girin.');
        setSonuc(null);
      }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      ...
      <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mb-6 font-medium">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-800 dark:text-slate-300">Matris Çarpımı Hesaplama</span>
      </div>

      {/* 2. BAŞLIK VE AÇIKLAMA */}
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">Matris Çarpımı Hesaplama 2026 - Ücretsiz ve Hızlı Sonuçlar</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          2x2 ve 3x3 boyutundaki iki matrisin çarpımını adım adım ve ayrıntılı sonuç matrisiyle hesaplayın.
        </p>
      </div>
          ...
          <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">1. Matris A (Satırlar virgülle, elemanlar boşlukla)</label>
                  <input
                    type="text"
                    value={val1}
                    onChange={(e) => { setVal1(e.target.value); setError(null); }}
                    placeholder="1 2, 3 4"
                    className="w-full bg-slate-50 dark:bg-slate-800 border border-black/10 dark:border-white/10 rounded-xl px-4 py-3 text-slate-700 dark:text-white font-medium focus:bg-white dark:focus:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-[#0056b3]/20 dark:focus:ring-blue-500/30 transition-all font-mono"
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">2. Matris B (Satırlar virgülle, elemanlar boşlukla)</label>
                  <input
                    type="text"
                    value={val2}
                    onChange={(e) => { setVal2(e.target.value); setError(null); }}
                    placeholder="5 6, 7 8"
                    className="w-full bg-slate-50 dark:bg-slate-800 border border-black/10 dark:border-white/10 rounded-xl px-4 py-3 text-slate-700 dark:text-white font-medium focus:bg-white dark:focus:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-[#0056b3]/20 dark:focus:ring-blue-500/30 transition-all font-mono"
                  />
                </div>
                  ...
                  {sonuc !== null && (
                    <div className="mt-4 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-black/5 dark:border-white/5">
                      ...
                      <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
                        Birinci matrisin satırları ile ikinci matrisin sütunlarının skaler çarpımlarının toplamından oluşan yeni matristir.
                      </p>
                    </div>
                  )}
                </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 mb-8 space-y-6 text-slate-700 dark:text-slate-300 leading-relaxed shadow-sm">
        ...
        <div>
          <h4 className="text-base font-bold text-slate-900 dark:text-white mb-2">Matris Çarpımı Hesaplama Nedir ve Nasıl Hesaplanır?</h4>
          <p className="text-sm">
            Matris Çarpımı Hesaplama, matematik ve geometri alanında sıkça başvurulan temel hesaplama disiplinlerinden biridir. 2x2 ve 3x3 boyutundaki iki matrisin çarpımını adım adım ve ayrıntılı sonuç matrisiyle hesaplayın. Hesaplama aracımız girilen tüm sayısal parametreleri analiz ederek kurallara uygun biçimde sonuçlandırır.
          </p>
        </div>
        ...
        <div>
          ...
          <div className="space-y-3 text-sm">
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">Matris çarpımı için temel kural nedir?</h5>
              <p>Birinci matrisin sütun sayısı ile ikinci matrisin satır sayısı birbirine eşit olmak zorundadır.</p>
            </div>
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">Matris çarpımında değişme özelliği var mıdır?</h5>
              <p>Genel olarak A × B ≠ B × A olup matris çarpımında değişme özelliği bulunmaz.</p>
            </div>
          </div>
        </div>
      </div>

            <div className="mt-8"><RelatedTools currentToolId="matris-carpimi-hesaplama" category="matematik" /></div>
<Disclaimer />
    </div>
  );
}
