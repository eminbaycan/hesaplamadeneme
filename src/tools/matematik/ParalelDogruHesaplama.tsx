...
export default function ParalelDogruHesaplama() {
  ...
  const hesapla = () => {
    ...
    setError(null);
      ...
      const m = parseFloat(val1) || 0;
      const x0 = parseFloat(val2) || 0;
      const y0 = parseFloat(val3) || 0;
      const c = y0 - (m * x0);
      setSonuc(`y = ${m}x ${c >= 0 ? '+ ' + c : '- ' + Math.abs(c)}`);
      setDetay(`Paralel Eğim m = ${m}, Nokta (${x0}, ${y0}) için y - ${y0} = ${m}(x - ${x0})`);
  };
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      ...
      <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mb-6 font-medium">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-800 dark:text-slate-300">Paralel Doğru Hesaplama</span>
      </div>

      {/* 2. BAŞLIK VE AÇIKLAMA */}
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">Paralel Doğru Hesaplama 2026 - Ücretsiz ve Hızlı Sonuçlar</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          Verilen bir doğruya (y = mx + b) paralel olan ve belirli bir noktadan (x0, y0) geçen yeni doğrunun denklemini bulun.
        </p>
      </div>
          ...
          <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">Eğim (m)</label>
                  ...
                  {sonuc !== null && (
                    <div className="mt-4 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-black/5 dark:border-white/5">
                      ...
                      <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
                        Aynı eğime sahip ve verilen noktadan geçen paralel doğrunun matematiksel denklemidir.
                      </p>
                    </div>
                  )}
                </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 mb-8 space-y-6 text-slate-700 dark:text-slate-300 leading-relaxed shadow-sm">
        ...
        <div>
          <h4 className="text-base font-bold text-slate-900 dark:text-white mb-2">Paralel Doğru Hesaplama Nedir ve Nasıl Hesaplanır?</h4>
          <p className="text-sm">
            Paralel Doğru Hesaplama, matematik ve geometri alanında sıkça başvurulan temel hesaplama disiplinlerinden biridir. Verilen bir doğruya (y = mx + b) paralel olan ve belirli bir noktadan (x0, y0) geçen yeni doğrunun denklemini bulun. Hesaplama aracımız girilen tüm sayısal parametreleri analiz ederek kurallara uygun biçimde sonuçlandırır.
          </p>
        </div>
        ...
        <div>
          ...
          <div className="space-y-3 text-sm">
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">İki doğrunun paralel olma şartı nedir?</h5>
              <p>Analitik düzlemde iki doğrunun birbirine paralel olması için eğimlerinin eşit olması (m₁ = m₂) gerekir.</p>
            </div>
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">Paralel doğrular kesişir mi?</h5>
              <p>Öklid geometrisinde eğimleri eşit ve sabit terimleri farklı olan paralel doğrular hiçbir zaman kesişmez.</p>
            </div>
          </div>
        </div>
      </div>

            <div className="mt-8"><RelatedTools currentToolId="paralel-dogru-hesaplama" category="matematik" /></div>
<Disclaimer />
    </div>
  );
}
