...
export default function DikdortgenUcgenDaireAlanHesaplama() {
  const [val1, setVal1] = useState<string>('');
  const [val2, setVal2] = useState<string>('6');
  ...
  const hesapla = () => {
    ...
    setError(null);
      ...
      if (a <= 0) {
        setError('*Lütfen geçerli pozitif bir boyut girin.');
        setSonuc(null);
        return;
      }
      if (val3 === 'dikdortgen') {
        const alan = a * b;
        setSonuc(`${alan.toLocaleString('tr-TR')} birim²`);
        setDetay(`Dikdörtgen Alanı = ${a} × ${b} = ${alan} birim²`);
      } else if (val3 === 'ucgen') {
        const alan = (a * b) / 2;
        setSonuc(`${alan.toLocaleString('tr-TR')} birim²`);
        setDetay(`Üçgen Alanı = (${a} × ${b}) / 2 = ${alan} birim²`);
      } else {
        const alan = Math.PI * a * a;
        setSonuc(`${alan.toFixed(2)} birim²`);
        setDetay(`Daire Alanı = π × ${a}² ≈ ${alan.toFixed(4)} birim² (π ≈ 3.14159)`);
      }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      ...
      <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mb-6 font-medium">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-800 dark:text-slate-300">Dikdörtgen, Üçgen ve Daire Alan Hesaplama</span>
      </div>

      {/* 2. BAŞLIK VE AÇIKLAMA */}
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">Dikdörtgen, Üçgen ve Daire Alan Hesaplama 2026 - Ücretsiz ve Hızlı Sonuçlar</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          Dikdörtgen, üçgen ve daire geometrik şekillerinin alanlarını doğru formüller ve kesin hassasiyetle hesaplayın.
        </p>
      </div>
      ...
      <div className="calculator-container mb-8 relative border border-slate-200 dark:border-slate-700 rounded-2xl p-5 sm:p-8 mt-8">
        <div className="absolute -top-3.5 left-4 sm:left-8 bg-[#eef2f7] dark:bg-slate-950 px-4">
          <h2 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
            ...
            Dikdörtgen, Üçgen ve Daire Alan Hesaplama
          </h2>
        </div>
          ...
          <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">Genişlik / Taban / Yarıçap (r)</label>
                  <input
                    type="number"
                    value={val1}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">Yükseklik (Üçgen/Dikdörtgen için)</label>
                  <input
                    type="number"
                    value={val2}
                  ...
                  {sonuc !== null && (
                    <div className="mt-4 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-black/5 dark:border-white/5">
                      ...
                      <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
                        Hesaplanan değer, seçilen geometrik cismin iki boyutlu düzlemde kapladığı toplam yüzey alanını ifade eder.
                      </p>
                    </div>
                  )}
                </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 mb-8 space-y-6 text-slate-700 dark:text-slate-300 leading-relaxed shadow-sm">
        ...
        <div>
          <h4 className="text-base font-bold text-slate-900 dark:text-white mb-2">Dikdörtgen, Üçgen ve Daire Alan Hesaplama Nedir ve Nasıl Hesaplanır?</h4>
          <p className="text-sm">
            Dikdörtgen, Üçgen ve Daire Alan Hesaplama, matematik ve geometri alanında sıkça başvurulan temel hesaplama disiplinlerinden biridir. Dikdörtgen, üçgen ve daire geometrik şekillerinin alanlarını doğru formüller ve kesin hassasiyetle hesaplayın. Hesaplama aracımız girilen tüm sayısal parametreleri analiz ederek kurallara uygun biçimde sonuçlandırır.
          </p>
        </div>
        ...
        <div>
          ...
          <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-xl font-mono text-sm border border-slate-200 dark:border-slate-700">
            {"Dikdörtgen: a × b | Üçgen: (Taban × Yükseklik) / 2 | Daire: π × r²"}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-3 text-sm">
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">Daire alanında pi sayısı kaç alınır?</h5>
              <p>Sistemimiz en yüksek doğruluk için Math.PI (3.14159265...) değerini kullanır.</p>
            </div>
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">Üçgen alanında yükseklik hangi kenara aittir?</h5>
              <p>Yükseklik (h) doğrudan girilen taban kenarına indirilen dikmenin uzunluğudur.</p>
            </div>
          </div>
        </div>
      </div>

            <div className="mt-8"><RelatedTools currentToolId="dikdortgen-ucgen-daire-alan-hesaplama" category="matematik" /></div>
<Disclaimer />
    </div>
  );
}
