...
export default function IkiNoktaArasiMesafeHesaplama() {
  ...
  const hesapla = () => {
    ...
    setError(null);
      ...
      const x1 = parseFloat(val1) || 0;
      const y1 = parseFloat(val2) || 0;
      const x2 = parseFloat(val3) || 0;
      const y2Elem = document.getElementById('y2_input') as HTMLInputElement;
      const y2 = y2Elem ? parseFloat(y2Elem.value) || 0 : 4;
      const dist = Math.sqrt(Math.pow(x2 - x1, 2) + Math.pow(y2 - y1, 2));
      setSonuc(`${dist.toFixed(4)} birim`);
      setDetay(`Öklid Uzaklığı: √[(${x2}-${x1})² + (${y2}-${y1})²] = ${dist.toFixed(4)}`);
  };
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      ...
      <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mb-6 font-medium">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-800 dark:text-slate-300">İki Nokta Arası Mesafe Hesaplama</span>
      </div>

      {/* 2. BAŞLIK VE AÇIKLAMA */}
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">İki Nokta Arası Mesafe Hesaplama 2026 - Ücretsiz ve Hızlı Sonuçlar</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          Analitik düzlemde (2D ve 3D) verilen iki koordinat noktası arasındaki Öklid mesafesini adım adım hesaplayın.
        </p>
      </div>
      ...
      <div className="calculator-container mb-8 relative border border-slate-200 dark:border-slate-700 rounded-2xl p-5 sm:p-8 mt-8">
        <div className="absolute -top-3.5 left-4 sm:left-8 bg-[#eef2f7] dark:bg-slate-950 px-4">
          <h2 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <ToolIcon name="alan" size={20} className="text-[#0056b3] dark:text-blue-400" />
          ...
          <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">X₁</label>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">Y₁</label>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">Y₂</label>
                  <input type="number" defaultValue="4" id="y2_input" className="w-full bg-slate-50 dark:bg-slate-800 border border-black/10 dark:border-white/10 rounded-xl px-4 py-3 text-slate-700 dark:text-white text-sm" />
                </div>
              </div>
                  ...
                  {sonuc !== null && (
                    <div className="mt-4 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-black/5 dark:border-white/5">
                      ...
                      <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
                        Analitik geometride iki koordinat noktası arasındaki en kısa doğrusal kuş uçuşu uzaklıktır.
                      </p>
                    </div>
                  )}
                </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 mb-8 space-y-6 text-slate-700 dark:text-slate-300 leading-relaxed shadow-sm">
        ...
        <div>
          <h4 className="text-base font-bold text-slate-900 dark:text-white mb-2">İki Nokta Arası Mesafe Hesaplama Nedir ve Nasıl Hesaplanır?</h4>
          <p className="text-sm">
            İki Nokta Arası Mesafe Hesaplama, matematik ve geometri alanında sıkça başvurulan temel hesaplama disiplinlerinden biridir. Analitik düzlemde (2D ve 3D) verilen iki koordinat noktası arasındaki Öklid mesafesini adım adım hesaplayın. Hesaplama aracımız girilen tüm sayısal parametreleri analiz ederek kurallara uygun biçimde sonuçlandırır.
          </p>
        </div>
        ...
        <div>
          ...
          <div className="space-y-3 text-sm">
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">İki nokta arası mesafe negatif olabilir mi?</h5>
              <p>Hayır, geometrik mesafe karekök alma prensibi gereği her zaman pozitif veya sıfırdır.</p>
            </div>
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">Pisagor teoremi ile ilişkisi nedir?</h5>
              <p>İki nokta arasındaki yatay ve dikey farklar dik üçgenin dik kenarlarını, aralarındaki mesafe ise hipotenüsü oluşturur.</p>
            </div>
          </div>
        </div>
      </div>

            <div className="mt-8"><RelatedTools currentToolId="iki-nokta-arasi-mesafe-hesaplama" category="matematik" /></div>
<Disclaimer />
    </div>
  );
}
