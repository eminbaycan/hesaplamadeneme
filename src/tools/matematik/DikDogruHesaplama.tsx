...
export default function DikDogruHesaplama() {
  ...
  const hesapla = () => {
    ...
    setError(null);
      ...
      const m1 = parseFloat(val1);
      if (m1 === 0 || isNaN(m1)) {
        setError('*Eğim 0 olamaz.');
        setSonuc(null);
        return;
      }
      const x0 = parseFloat(val2) || 0;
      const y0 = parseFloat(val3) || 0;
      const m2 = -1 / m1;
      const c = y0 - (m2 * x0);
      setSonuc(`y = ${m2.toFixed(2)}x ${c >= 0 ? '+ ' + c.toFixed(2) : '- ' + Math.abs(c).toFixed(2)}`);
      setDetay(`Dik Eğim m₂ = -1 / ${m1} = ${m2.toFixed(4)}`);
  };
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      ...
      <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mb-6 font-medium">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-800 dark:text-slate-300">Dik Doğru Hesaplama</span>
      </div>

      {/* 2. BAŞLIK VE AÇIKLAMA */}
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">Dik Doğru Hesaplama 2026 - Ücretsiz ve Hızlı Sonuçlar</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          Verilen bir doğruya dik olan (eğimleri çarpımı m1 * m2 = -1) ve belirli bir noktadan geçen dik doğrunun denklemini bulun.
        </p>
      </div>
          ...
          <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">Mevcut Eğim (m₁ ≠ 0)</label>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">X₀ Noktası</label>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">Y₀ Noktası</label>
                  ...
                  {sonuc !== null && (
                    <div className="mt-4 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-black/5 dark:border-white/5">
                      ...
                      <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
                        Mevcut doğruyla 90 derecelik dik açı yapan ve verilen koordinattan geçen doğrunun denklemidir.
                      </p>
                    </div>
                  )}
                </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 mb-8 space-y-6 text-slate-700 dark:text-slate-300 leading-relaxed shadow-sm">
        ...
        <div>
          <h4 className="text-base font-bold text-slate-900 dark:text-white mb-2">Dik Doğru Hesaplama Nedir ve Nasıl Hesaplanır?</h4>
          <p className="text-sm">
            Dik Doğru Hesaplama, matematik ve geometri alanında sıkça başvurulan temel hesaplama disiplinlerinden biridir. Verilen bir doğruya dik olan (eğimleri çarpımı m1 * m2 = -1) ve belirli bir noktadan geçen dik doğrunun denklemini bulun. Hesaplama aracımız girilen tüm sayısal parametreleri analiz ederek kurallara uygun biçimde sonuçlandırır.
          </p>
        </div>
        ...
        <div>
          ...
          <div className="space-y-3 text-sm">
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">İki doğrunun dik kesişme şartı nedir?</h5>
              <p>Analitik düzlemde birbirine dik olan doğruların eğimleri çarpımı -1 dir (m₁ × m₂ = -1).</p>
            </div>
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">Eğimi 0 olan bir doğruya dik doğru nasıl yazılır?</h5>
              <p>Eğimi 0 olan doğru yataydır (y = b); buna dik olan doğru düşeydir ve x = sabit denklemine sahiptir.</p>
            </div>
          </div>
        </div>
      </div>

            <div className="mt-8"><RelatedTools currentToolId="dik-dogru-hesaplama" category="matematik" /></div>
<Disclaimer />
    </div>
  );
}
