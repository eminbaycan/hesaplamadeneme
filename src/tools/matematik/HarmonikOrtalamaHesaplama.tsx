...
export default function HarmonikOrtalamaHesaplama() {
  ...
  const hesapla = () => {
    ...
    setError(null);
      ...
      const parts = val1.split(/[,\s]+/).map(v => parseFloat(v)).filter(v => !isNaN(v) && v > 0);
      if (parts.length === 0) {
        setError('*Lütfen geçerli pozitif sayılar girin.');
        setSonuc(null);
        return;
      }
      const sumInv = parts.reduce((acc, val) => acc + (1 / val), 0);
      const harm = parts.length / sumInv;
      setSonuc(`H = ${harm.toFixed(4)}`);
      setDetay(`${parts.length} adet veri için Harmonik Ortalama = ${parts.length} / (${sumInv.toFixed(4)})`);
  };
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      ...
      <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mb-6 font-medium">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-800 dark:text-slate-300">Harmonik Ortalama Hesaplama</span>
      </div>

      {/* 2. BAŞLIK VE AÇIKLAMA */}
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">Harmonik Ortalama Hesaplama 2026 - Ücretsiz ve Hızlı Sonuçlar</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          Hız, oran ve birim zaman hesaplamalarında kullanılan veri dizilerinin harmonik ortalamasını (H) anında hesaplayın.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-4">
          ...
          <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm">
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">Sayılar (Virgülle veya boşlukla ayırın)</label>
                <input
                  type="text"
                  value={val1}
                  ...
                  {sonuc !== null && (
                    <div className="mt-4 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-black/5 dark:border-white/5">
                      ...
                      <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
                        Birim zaman oranları ve hız ortalamalarında kullanılan rasyonel ters toplam ortalamasıdır.
                      </p>
                    </div>
                  )}
                </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 mb-8 space-y-6 text-slate-700 dark:text-slate-300 leading-relaxed shadow-sm">
        ...
        <div>
          <h4 className="text-base font-bold text-slate-900 dark:text-white mb-2">Harmonik Ortalama Hesaplama Nedir ve Nasıl Hesaplanır?</h4>
          <p className="text-sm">
            Harmonik Ortalama Hesaplama, matematik ve geometri alanında sıkça başvurulan temel hesaplama disiplinlerinden biridir. Hız, oran ve birim zaman hesaplamalarında kullanılan veri dizilerinin harmonik ortalamasını (H) anında hesaplayın. Hesaplama aracımız girilen tüm sayısal parametreleri analiz ederek kurallara uygun biçimde sonuçlandırır.
          </p>
        </div>
        ...
        <div>
          ...
          <div className="space-y-3 text-sm">
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">Harmonik ortalama ne zaman kullanılır?</h5>
              <p>Ortalama hız, finansal çarpanlar ve oranların ortalamasını alırken aritmetik ortalamaya göre daha doğru sonuç verir.</p>
            </div>
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">Negatif sayılarla harmonik ortalama hesaplanır mı?</h5>
              <p>Harmonik ortalama genel olarak kesin pozitif veri setleri için tanımlıdır.</p>
            </div>
          </div>
        </div>
      </div>

            <div className="mt-8"><RelatedTools currentToolId="harmonik-ortalama-hesaplama" category="matematik" /></div>
<Disclaimer />
    </div>
  );
}
