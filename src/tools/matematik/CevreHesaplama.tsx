...
export default function CevreHesaplama() {
  const [kisaKenar, setKisaKenar] = useState<number | ''>('');
  const [copied, setCopied] = useState<boolean>(false);
  const [uzunKenar, setUzunKenar] = useState<number | ''>('');
  ...
  const hesapla = () => {
    ...
    // Dikdörtgen çevre formülü: 2 * (a + b)
    setSonuc(2 * (Number(kisaKenar) + Number(uzunKenar)));
  };
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      ...
      <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mb-6 font-medium">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-800 dark:text-slate-300">Çevre Hesaplama</span>
      </div>

      {/* 2. BAŞLIK VE AÇIKLAMA */}
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">Çevre Hesaplama 2026 - Ücretsiz ve Hızlı Sonuçlar</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          Dikdörtgen veya kare gibi temel geometrik şekillerin çevresini hızlı ve hatasız bir şekilde hesaplayın.
        </p>
      </div>
            ...
            <div className="space-y-5">
              <div>
                <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">
                  Kısa Kenar <span className="text-red-500">*</span>
                </label>
                <input 
                  type="number" 
                  value={kisaKenar}
                  onChange={(e) => {
                    setKisaKenar(e.target.value === '' ? '' : Number(e.target.value));
                    setError(null);
                  }}
                  placeholder="Örn: 10" 
              ...
              <div>
                <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">
                  Uzun Kenar (Kare ise aynı değeri girin) <span className="text-red-500">*</span>
                </label>
                <input 
                  type="number" 
                  value={uzunKenar}
                  onChange={(e) => {
                    setUzunKenar(e.target.value === '' ? '' : Number(e.target.value));
                    setError(null);
                  }}
                  placeholder="Örn: 20" 
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-black/10 dark:border-white/10 rounded-xl px-4 py-3 text-slate-700 dark:text-white font-medium focus:bg-white dark:focus:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-[#0056b3]/20 dark:focus:ring-blue-500/30 transition-all"
            ...
            <div className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-3xl p-6 md:p-8 flex-1 relative overflow-hidden shadow-lg border border-black/5 dark:border-slate-800">
              ...
              <div className="relative z-10 flex flex-col h-full">
                ...
                <div className="flex-1 flex flex-col justify-center">
                  <p className="text-slate-500 dark:text-slate-400 text-sm mb-1 font-medium">Çevre Uzunluğu</p>
                  ...
                  {sonuc !== null && (
                    <div className="mt-6 p-4 bg-slate-50 dark:bg-slate-800 rounded-xl border border-black/5 dark:border-white/5">
                      ...
                      <p className="text-xs text-slate-600 dark:text-slate-400">
                        Hesaplanan {sonuc.toLocaleString('tr-TR')} birimlik değer, {kisaKenar} ve {uzunKenar} birim uzunluğundaki kenarlara sahip bu geometrik şeklin etrafını tam olarak bir tur dönmek için gereken toplam mesafe miktarını ifade eder.
                      </p>
                    </div>
                  )}
                </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm">
        ...
        <div className="prose prose-sm md:prose-base max-w-none text-slate-600 dark:text-slate-400 space-y-6 leading-relaxed">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <h4 className="text-[17px] font-bold text-slate-800 dark:text-slate-200 mb-2">Çevre uzunluğu nedir?</h4>
              <p>
                Bir geometrik şekli çevreleyen dış sınır çizgilerinin toplam uzunluğudur. Bir bahçenin etrafına çekilecek çit uzunluğunu hesaplamak gibi örneklendirilebilir.
              </p>
            </div>
            <div>
              <h4 className="text-[17px] font-bold text-slate-800 dark:text-slate-200 mb-2">Dikdörtgenin çevresi nasıl bulunur?</h4>
              <p>
                Dikdörtgende karşılıklı kenarlar birbirine eşit olduğu için, bir kısa kenar ile bir uzun kenarın toplamının iki ile çarpılması çevre uzunluğunu verir.
              </p>
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-black/5 dark:border-white/5">
            ...
            <div className="space-y-6 bg-slate-50 dark:bg-slate-800/30 p-4 md:p-6 rounded-2xl border border-black/5 dark:border-white/5">
              ...
              <div>
                <div className="font-semibold text-slate-800 dark:text-slate-200 mb-1">Dikdörtgen Çevre Formülü:</div>
        ...
        <div className="prose prose-sm md:prose-base max-w-none text-slate-600 dark:text-slate-400 space-y-8 leading-relaxed">
          ...
          <section className="pt-4 border-t border-black/5 dark:border-white/5">
            ...
            <div className="space-y-6">
              <div>
                <h5 className="font-bold text-slate-800 dark:text-slate-200 mb-2">Çevre Hesaplama aracı ücretli midir?</h5>
      ...
      <RelatedTools category="matematik" currentToolId="cevre-hesaplama" />
