...
export default function SayiTabaniDonusturme() {
  const [val1, setVal1] = useState<string>('');
  const [val2, setVal2] = useState<string>('10');
  const [val3, setVal3] = useState<string>('2');
  ...
  const hesapla = () => {
    ...
    setError(null);
      ...
      try {
        const src = parseInt(val2 || '10', 10);
        const tgt = parseInt(val3 || '2', 10);
        const dec = parseInt(val1.trim(), src);
        if (isNaN(dec)) throw new Error('Geçersiz sayı');
        const res = dec.toString(tgt).toUpperCase();
        setSonuc(res);
        setDetay(`Kaynak Sayı: ${val1} (Taban: ${src}) ➔ Hedef Sayı: ${res} (Taban: ${tgt}) | Onluk Karşılık: ${dec}`);
      } catch {
        setError('*Girdiğiniz sayı seçilen kaynak tabana uygun değil.');
        setSonuc(null);
      }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      ...
      <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mb-6 font-medium">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-800 dark:text-slate-300">Sayı Tabanı Dönüştürme</span>
      </div>

      {/* 2. BAŞLIK VE AÇIKLAMA */}
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">Sayı Tabanı Dönüştürme 2026 - Ücretsiz ve Hızlı Sonuçlar</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          İkili (Binary), Sekizli (Octal), Onlu (Decimal) ve Onaltılı (Hexadecimal) gibi farklı sayı tabanları arasında hızlı ve hatasız dönüşüm yapın.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-4">
          ...
          <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm">
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">Dönüştürülecek Sayı</label>
                <input
                  type="text"
                  value={val1}
                  onChange={(e) => { setVal1(e.target.value); setError(null); }}
                  placeholder="Örn: 255 veya 101010"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">Kaynak Taban</label>
                  <select
                    value={val2}
                    onChange={(e) => setVal2(e.target.value)}
                    className="w-full bg-slate-50 dark:bg-slate-800 border border-black/10 dark:border-white/10 rounded-xl px-4 py-3 text-slate-700 dark:text-white font-medium focus:bg-white dark:focus:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-[#0056b3]/20 dark:focus:ring-blue-500/30 transition-all"
                  >
                    <option value="2">2 (İkili / Binary)</option>
                    <option value="8">8 (Sekizli / Octal)</option>
                    <option value="10">10 (Onluk / Decimal)</option>
                    <option value="16">16 (Onaltılık / Hexadecimal)</option>
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">Hedef Taban</label>
                  <select
                    value={val3}
                  ...
                  {sonuc !== null && (
                    <div className="mt-4 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-black/5 dark:border-white/5">
                      ...
                      <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
                        Girdiğiniz kaynak sayı, seçtiğiniz hedef sayı tabanındaki eşdeğer matematiksel basamak dizilimine çevrilmiştir.
                      </p>
                    </div>
                  )}
                </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 mb-8 space-y-6 text-slate-700 dark:text-slate-300 leading-relaxed shadow-sm">
        ...
        <div>
          <h4 className="text-base font-bold text-slate-900 dark:text-white mb-2">Sayı Tabanı Dönüştürme Nedir ve Nasıl Hesaplanır?</h4>
          <p className="text-sm">
            Sayı Tabanı Dönüştürme, matematik ve geometri alanında sıkça başvurulan temel hesaplama disiplinlerinden biridir. İkili (Binary), Sekizli (Octal), Onlu (Decimal) ve Onaltılı (Hexadecimal) gibi farklı sayı tabanları arasında hızlı ve hatasız dönüşüm yapın. Hesaplama aracımız girilen tüm sayısal parametreleri analiz ederek kurallara uygun biçimde sonuçlandırır.
          </p>
        </div>
        ...
        <div>
          ...
          <div className="space-y-3 text-sm">
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">Binary (İkili) taban nedir?</h5>
              <p>Yalnızca 0 ve 1 rakamlarından oluşan, bilgisayar donanımlarının durumlarını temsil eden 2 tabanlı sayı sistemidir.</p>
            </div>
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">Hexadecimal (16 tabanı) neden kullanılır?</h5>
              <p>Binary sayıların çok uzun yazılmasını engellemek için 4 bitlik blokları tek bir karakterle (0-F) temsil etmek amacıyla kullanılır.</p>
            </div>
          </div>
        </div>
      </div>

            <div className="mt-8"><RelatedTools currentToolId="sayi-tabani-donusturme" category="matematik" /></div>
<Disclaimer />
    </div>
  );
}
