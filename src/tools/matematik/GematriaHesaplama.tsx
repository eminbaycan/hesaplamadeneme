...
export default function GematriaHesaplama() {
  ...
  const hesapla = () => {
    ...
    setError(null);
      ...
      let sum = 0;
      const clean = val1.toUpperCase();
      for (let i = 0; i < clean.length; i++) {
        const code = clean.charCodeAt(i);
        if (code >= 65 && code <= 90) sum += (code - 64);
      }
      setSonuc(`Toplam: ${sum}`);
      setDetay(`Analiz edilen karakter sayısı: ${clean.length}, Basit Alfabetik Gematria Toplamı: ${sum}`);
  };
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      ...
      <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mb-6 font-medium">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-800 dark:text-slate-300">Gematria Hesaplama</span>
      </div>

      {/* 2. BAŞLIK VE AÇIKLAMA */}
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">Gematria Hesaplama 2026 - Ücretsiz ve Hızlı Sonuçlar</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          İbranice, İngilizce, Türkçe ve Ebced alfabe değerlerine göre metinlerin sayısal ve numerolojik Gematria değerlerini hesaplayın.
        </p>
      </div>
      ...
      <div className="calculator-container mb-8 relative border border-slate-200 dark:border-slate-700 rounded-2xl p-5 sm:p-8 mt-8">
        <div className="absolute -top-3.5 left-4 sm:left-8 bg-[#eef2f7] dark:bg-slate-950 px-4">
          <h2 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <ToolIcon name="altin-oran" size={20} className="text-[#0056b3] dark:text-blue-400" />
            Gematria Hesaplama
          </h2>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-4">
          ...
          <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm">
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">Metin / Kelime</label>
                <input
                  type="text"
                  value={val1}
                  onChange={(e) => { setVal1(e.target.value); setError(null); }}
                  placeholder="Örn: HESAPLAMA"
                  ...
                  {sonuc !== null && (
                    <div className="mt-4 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-black/5 dark:border-white/5">
                      ...
                      <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
                        Girilen kelimenin alfabedeki harf sıralarına göre elde edilen sayısal toplam değeridir.
                      </p>
                    </div>
                  )}
                </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 mb-8 space-y-6 text-slate-700 dark:text-slate-300 leading-relaxed shadow-sm">
        ...
        <div>
          <h4 className="text-base font-bold text-slate-900 dark:text-white mb-2">Gematria Hesaplama Nedir ve Nasıl Hesaplanır?</h4>
          <p className="text-sm">
            Gematria Hesaplama, matematik ve geometri alanında sıkça başvurulan temel hesaplama disiplinlerinden biridir. İbranice, İngilizce, Türkçe ve Ebced alfabe değerlerine göre metinlerin sayısal ve numerolojik Gematria değerlerini hesaplayın. Hesaplama aracımız girilen tüm sayısal parametreleri analiz ederek kurallara uygun biçimde sonuçlandırır.
          </p>
        </div>
        ...
        <div>
          ...
          <div className="space-y-3 text-sm">
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">Gematria sistemi nedir?</h5>
              <p>Harflere karşılık gelen sayısal değerlerin toplanmasıyla metinlerin sayısal analizini yapan köklü bir sistemdir.</p>
            </div>
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">Ebced hesabı ile farkı nedir?</h5>
              <p>Ebced hesabı Arap alfabesindeki harflerin tarihsel değerlerine dayanırken Gematria İbranice ve Latin harflerine uygulanır.</p>
            </div>
          </div>
        </div>
      </div>

            <div className="mt-8"><RelatedTools currentToolId="gematria-hesaplama" category="matematik" /></div>
<Disclaimer />
    </div>
  );
}
