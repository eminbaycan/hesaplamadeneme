...
export default function LineerCebirHesaplama() {
  ...
  const hesapla = () => {
    ...
    setError(null);
      ...
      const u = val1.split(/[,\s]+/).map(Number).filter(v => !isNaN(v));
      const v = val2.split(/[,\s]+/).map(Number).filter(v => !isNaN(v));
      if (u.length === 0) {
        setError('*Lütfen geçerli vektör elemanları girin.');
        setSonuc(null);
        return;
      }
      const normU = Math.sqrt(u.reduce((acc, x) => acc + x * x, 0));
      let dot = 0;
      const len = Math.min(u.length, v.length);
      for (let i = 0; i < len; i++) dot += u[i] * (v[i] || 0);
      setSonuc(`||u|| = ${normU.toFixed(2)} | u·v = ${dot}`);
      setDetay(`u normu: ${normU.toFixed(4)}, İç Çarpım (${len} eleman): ${dot}`);
  };
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      ...
      <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mb-6 font-medium">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-800 dark:text-slate-300">Lineer Cebir Hesaplama</span>
      </div>

      {/* 2. BAŞLIK VE AÇIKLAMA */}
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">Lineer Cebir Hesaplama 2026 - Ücretsiz ve Hızlı Sonuçlar</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          Lineer denklem sistemleri, vektör işlemleri, determinant ve matris rankı hesaplamalarını pratik şekilde gerçekleştirin.
        </p>
      </div>
          ...
          <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">1. Vektör u (x, y veya x, y, z)</label>
                  <input
                    type="text"
                    value={val1}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">2. Vektör v (x2, y2 veya x2, y2, z2)</label>
                  <input
                    type="text"
                    value={val2}
                  ...
                  {sonuc !== null && (
                    <div className="mt-4 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-black/5 dark:border-white/5">
                      ...
                      <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
                        Vektörlerin uzunlukları (normu) ve aralarındaki skaler yönelim ilişkisini belirten iç çarpım değeridir.
                      </p>
                    </div>
                  )}
                </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 mb-8 space-y-6 text-slate-700 dark:text-slate-300 leading-relaxed shadow-sm">
        ...
        <div>
          <h4 className="text-base font-bold text-slate-900 dark:text-white mb-2">Lineer Cebir Hesaplama Nedir ve Nasıl Hesaplanır?</h4>
          <p className="text-sm">
            Lineer Cebir Hesaplama, matematik ve geometri alanında sıkça başvurulan temel hesaplama disiplinlerinden biridir. Lineer denklem sistemleri, vektör işlemleri, determinant ve matris rankı hesaplamalarını pratik şekilde gerçekleştirin. Hesaplama aracımız girilen tüm sayısal parametreleri analiz ederek kurallara uygun biçimde sonuçlandırır.
          </p>
        </div>
        ...
        <div>
          ...
          <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-xl font-mono text-sm border border-slate-200 dark:border-slate-700">
            {"Det(A) = ad - bc | Vektör İç Çarpımı: u · v = Σ (uᵢ × vᵢ)"}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-3 text-sm">
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">Vektör normu neyi ifade eder?</h5>
              <p>Bir vektörün başlangıç noktasından bitiş noktasına olan geometrik uzunluğunu (büyüklüğünü) ifade eder.</p>
            </div>
            <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
              <h5 className="font-bold text-slate-900 dark:text-white mb-1">Determinant sıfır çıkarsa ne anlama gelir?</h5>
              <p>Matrisin tersinin alınamayacağını (tekil matris) ve denklem sisteminin tek bir çözümü olmadığını gösterir.</p>
            </div>
          </div>
        </div>
      </div>

            <div className="mt-8"><RelatedTools currentToolId="lineer-cebir-hesaplama" category="matematik" /></div>
<Disclaimer />
    </div>
  );
}
