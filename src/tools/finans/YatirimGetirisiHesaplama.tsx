...
import { ChevronRight, RefreshCw, Copy, Info, TrendingUp, AlertCircle, Check } from 'lucide-react';
...
export default function YatirimGetirisiHesaplama() {
  const [baslangicTutar, setBaslangicTutar] = useState<number | ''>('');
  const [bitisTutar, setBitisTutar] = useState<number | ''>('');
  const [sureAy, setSureAy] = useState<number | ''>('');
  ...
  const handleCalculate = () => {
    if (baslangicTutar === '' || bitisTutar === '') {
      setError('*Lütfen başlangıç ve bitiş tutarı alanlarını doldurun.');
      setSonuc(null);
      return;
    }
    setError(null);

    const baslangic = Number(baslangicTutar);
    const bitis = Number(bitisTutar);
    const ay = typeof sureAy === 'number' && sureAy > 0 ? sureAy : 12;

    const netKarZarar = bitis - baslangic;
    const yuzdeGetiri = (netKarZarar / baslangic) * 100;
    const yillikGetiri = yuzdeGetiri * (12 / ay);
  ...
  const handleCopy = () => {
    if (!sonuc) return;
    const text = `Net Kâr/Zarar: ${sonuc.netKarZarar.toFixed(2)} ₺ | Toplam Getiri: %${sonuc.yuzdeGetiri.toFixed(2)}`;
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      ...
      <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mb-6 font-medium">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-800 dark:text-slate-300">Yatırım Getirisi (ROI) Hesaplama</span>
      </div>

      {/* 2. BAŞLIK VE AÇIKLAMA */}
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">Yatırım Getirisi (ROI) Hesaplama 2026 - Ücretsiz ve Hızlı Sonuçlar</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          Yatırımınızın başlangıç tutarı ve son değerine göre toplam kâr/zarar oranını ve yatırım getirisini (ROI) kolayca hesaplayın.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-4">
            ...
            <div className="space-y-5">
              <div>
                <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">
                  Başlangıç Yatırım Tutarı (TL) <span className="text-red-500">*</span>
                </label>
                <input 
                  type="number" 
                  step="0.01"
                  value={baslangicTutar}
                  onChange={(e) => {
                    setBaslangicTutar(e.target.value === '' ? '' : Number(e.target.value));
                    setError(null);
                  }}
              ...
              <div>
                <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">
                  Bitiş / Güncel Değer (TL) <span className="text-red-500">*</span>
                </label>
                <input 
                  type="number" 
                  step="0.01"
                  value={bitisTutar}
                  onChange={(e) => {
                    setBitisTutar(e.target.value === '' ? '' : Number(e.target.value));
                    setError(null);
                  }}
              ...
              <div>
                ...
                <input 
                  ...
                  value={sureAy}
                  onChange={(e) => {
                    setSureAy(e.target.value === '' ? '' : Number(e.target.value));
                    setError(null);
                  }}
                  placeholder="Örn: 12" 
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-black/10 dark:border-white/10 rounded-xl px-4 py-3 text-slate-700 dark:text-white font-medium focus:bg-white dark:focus:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-[#0056b3]/20 dark:focus:ring-blue-500/30 focus:border-[#0056b3] dark:focus:border-blue-500 transition-all"
          ...
          <div className="lg:col-span-5 flex flex-col">
            <div className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-3xl p-6 md:p-8 flex-1 relative overflow-hidden shadow-lg border border-black/5 dark:border-slate-800 flex flex-col justify-between">
              <div>
                <h3 className="text-sm font-bold uppercase tracking-widest text-slate-800 dark:text-white mb-6">Getiri Analizi</h3>

                {sonuc !== null ? (
                  <div className="space-y-4 mb-6">
                    <div>
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">Net Kâr / Zarar Tutar</span>
                    ...
                    <div>
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">Toplam Yatırım Getirisi (ROI)</span>
                      <div className={`text-3xl font-black ${sonuc.yuzdeGetiri >= 0 ? 'text-[#0056b3] dark:text-blue-400' : 'text-rose-600 dark:text-rose-400'}`}>
                        {sonuc.yuzdeGetiri >= 0 ? '+' : ''}%{sonuc.yuzdeGetiri.toFixed(2)}
                      </div>
                    </div>
                  </div>
                ) : (
                  <div className="py-12 text-center text-slate-400 text-sm">
                    Yatırım değerlerini girip hesapla butonuna tıklayın.
                  </div>
                )}
              </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm space-y-6">
        <h3 className="text-xl font-bold text-slate-800 dark:text-white flex items-center gap-2 border-b border-black/5 dark:border-white/10 pb-4">
          ...
          Yatırım Getirisi (ROI) Hesaplama Rehberi ve Sıkça Sorulan Sorular
        </h3>

        <div className="prose prose-sm md:prose-base max-w-none text-slate-600 dark:text-slate-400 space-y-6 leading-relaxed">
          <div>
            <h4 className="text-[17px] font-bold text-slate-800 dark:text-slate-200 mb-2">Yatırım Getirisi (ROI) Nedir?</h4>
            <p>
              ROI (Return on Investment), bir yatırımın verimliliğini ve kârlılığını ölçmek için kullanılan finansal bir performans ölçütüdür. Başlangıç maliyetine kıyasla elde edilen net kâr yüzdesini gösterir.
            </p>
          </div>
          ...
          <div className="pt-4 border-t border-black/5 dark:border-white/5">
            ...
            <div className="space-y-4 bg-slate-50 dark:bg-slate-800/30 p-4 md:p-6 rounded-2xl border border-black/5 dark:border-white/5">
              <div>
                <div className="font-semibold text-slate-800 dark:text-slate-200 mb-1">ROI Hesaplama Formülü:</div>
                <div className="bg-white dark:bg-slate-900 border border-black/10 dark:border-white/10 px-4 py-2.5 rounded-xl font-mono text-sm text-[#0056b3] dark:text-blue-400 mb-2 overflow-x-auto">
                  Net Kâr = Bitiş Değeri - Başlangıç Maliyeti <br />
                  ROI (%) = (Net Kâr / Başlangıç Maliyeti) × 100
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-black/5 dark:border-white/5">
            ...
            <div className="space-y-4">
              <div>
                <h5 className="font-bold text-slate-800 dark:text-slate-200 mb-1">Yüksek ROI her zaman iyi midir?</h5>
                <p>Genellikle yüksek ROI olumlu bir göstergedir; ancak yatırımın içerdiği risk düzeyi ve vade süresi de mutlaka değerlendirilmelidir.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm mb-8">
        <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-6 flex items-center gap-2 border-b border-black/5 dark:border-white/10 pb-4">
          <Info className="text-[#0056b3] dark:text-blue-400 shrink-0" size={24} /> 
          Yatırım Getirisi (ROI) Hesaplama Hakkında Her Şey ve Sıkça Sorulan Sorular
        </h3>
        ...
        <div className="prose prose-sm md:prose-base max-w-none text-slate-600 dark:text-slate-400 space-y-8 leading-relaxed">
          ...
          <section className="pt-4 border-t border-black/5 dark:border-white/5">
            ...
            <div className="space-y-6">
              <div>
                <h5 className="font-bold text-slate-800 dark:text-slate-200 mb-2">Yatırım Getirisi (ROI) Hesaplama aracı ücretli midir?</h5>
      ...
      <RelatedTools category="finans" currentToolId="yatirim-getirisi-hesaplama" />
