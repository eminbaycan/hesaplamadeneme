...
import { ChevronRight, RefreshCw, Copy, Info, Coins, AlertCircle, Check } from 'lucide-react';
...
export default function AltinYatirimHesaplama() {
  const [altinTuru, setAltinTuru] = useState<string>('gram');
  ...
  const handleCalculate = () => {
    if (adet === '' || alisFiyati === '' || guncelFiyat === '') {
      setError('*Lütfen adet, alış fiyatı ve güncel fiyat alanlarını doldurun.');
      setSonuc(null);
      return;
    }
    setError(null);
    ...
    const toplamMaliyet = m * alis;
    const guncelDeger = m * guncel;
    const netKarZarar = guncelDeger - toplamMaliyet;
  ...
  const handleCopy = () => {
    if (!sonuc) return;
    const text = `Altın Yatırımı Toplam Değer: ${sonuc.guncelDeger.toFixed(2)} ₺ | Net Kâr/Zarar: ${sonuc.netKarZarar.toFixed(2)} ₺ (%${sonuc.getiriYuzdesi.toFixed(2)})`;
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      ...
      <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mb-6 font-medium">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-800 dark:text-slate-300">Altın Yatırım Kâr / Zarar Hesaplama</span>
      </div>

      {/* 2. BAŞLIK VE AÇIKLAMA */}
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">Altın Yatırım Kâr / Zarar Hesaplama 2026 - Ücretsiz ve Hızlı Sonuçlar</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          Gram, çeyrek veya bilezik gibi altın yatırımlarınızın alış ve güncel fiyatlarına göre getiri ve kâr durumunu hesaplayın.
        </p>
      </div>
          ...
          <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm">
            ...
            <div className="space-y-5">
              <div>
                ...
                <select
                  value={altinTuru}
                  onChange={(e) => setAltinTuru(e.target.value)}
                ...
                >
                  ...
                  <option value="ata">Ata Altın</option>
                </select>
              </div>
              ...
              <div>
                <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">
                  Alış Fiyatı (Birim Başına TL) <span className="text-red-500">*</span>
                </label>
                <input 
                  type="number" 
                  step="0.01"
              ...
              <div>
                <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">
                  Güncel / Satış Fiyatı (Birim Başına TL) <span className="text-red-500">*</span>
                </label>
                <input 
                  type="number" 
                  step="0.01"
              ...
              <div>
                ...
                {sonuc !== null ? (
                  <div className="space-y-4 mb-6">
                    <div>
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">Toplam Maliyet (TL)</span>
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm space-y-6">
        <h3 className="text-xl font-bold text-slate-800 dark:text-white flex items-center gap-2 border-b border-black/5 dark:border-white/10 pb-4">
          ...
          Altın Yatırımı Hesaplama Rehberi
        </h3>

        <div className="prose prose-sm md:prose-base max-w-none text-slate-600 dark:text-slate-400 space-y-6 leading-relaxed">
          <div>
            <h4 className="text-[17px] font-bold text-slate-800 dark:text-slate-200 mb-2">Altın Getirisi Nasıl Hesaplanır?</h4>
            <p>
              Altın yatırımlarında net kâr, güncel toplam değer ile toplam alış maliyeti arasındaki farktır. İşçilikli takılarda alış-satış makasının daha geniş olduğu unutulmamalıdır.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm mb-8">
        <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-6 flex items-center gap-2 border-b border-black/5 dark:border-white/10 pb-4">
          <Info className="text-[#0056b3] dark:text-blue-400 shrink-0" size={24} /> 
          Altın Yatırım Kâr / Zarar Hesaplama Hakkında Her Şey ve Sıkça Sorulan Sorular
        </h3>
        ...
        <div className="prose prose-sm md:prose-base max-w-none text-slate-600 dark:text-slate-400 space-y-8 leading-relaxed">
            ...
            <div className="space-y-4 bg-slate-50 dark:bg-slate-800/30 p-4 md:p-6 rounded-2xl border border-black/5 dark:border-white/5">
              <div>
                ...
                <div className="bg-white dark:bg-slate-900 border border-black/10 dark:border-white/10 px-4 py-2.5 rounded-xl font-mono text-sm text-[#0056b3] dark:text-blue-400 mb-2 overflow-x-auto">
                  Altın Kâr/Zarar = (Güncel Satış Fiyatı × Gram/Adet) - (Alış Fiyatı × Gram/Adet)
                </div>
              </div>
            </div>
          </div>
          ...
          <section className="pt-4 border-t border-black/5 dark:border-white/5">
            ...
            <div className="space-y-6">
              <div>
                <h5 className="font-bold text-slate-800 dark:text-slate-200 mb-2">Altın Yatırım Kâr / Zarar Hesaplama aracı ücretli midir?</h5>
      ...
      <RelatedTools category="finans" currentToolId="altin-yatirim-hesaplama" />
      <div className="mt-8">
