...
import { ChevronRight, RefreshCw, Copy, Info, Shield, AlertCircle, Check } from 'lucide-react';
...
export default function BesGetiriHesaplama() {
  const [aylikKatki, setAylikKatki] = useState<number | ''>('');
  const [sureYil, setSureYil] = useState<number | ''>('');
  const [beklenenGetiri, setBeklenenGetiri] = useState<number | ''>('');
  const [sonuc, setSonuc] = useState<{
    toplamKatki: number;
    devletKatkisi: number;
    tahminiGetiri: number;
  ...
  const handleCalculate = () => {
    if (aylikKatki === '' || sureYil === '') {
      setError('*Lütfen aylık katkı payı ve süre alanlarını doldurun.');
      setSonuc(null);
      return;
    }
    setError(null);

    const katki = Number(aylikKatki);
    const yil = Number(sureYil);
    const getiriOrani = Number(beklenenGetiri) / 100;

    const toplamKatki = katki * 12 * yil;
    // Devlet katkısı: Ödenen katkının %30'u
    const devletKatkisi = toplamKatki * 0.30;
    
    // Basit bileşik getiri tahmini
    let toplamBirikim = 0;
    let anaparaHavuzu = 0;
    for (let i = 0; i < yil; i++) {
      anaparaHavuzu += katki * 12;
      anaparaHavuzu *= (1 + getiriOrani);
    }
    const tahminiGetiri = anaparaHavuzu - (toplamKatki + devletKatkisi);
    toplamBirikim = anaparaHavuzu + devletKatkisi;

    setSonuc({
      toplamKatki,
      devletKatkisi,
      tahminiGetiri: Math.max(0, tahminiGetiri),
      toplamBirikim: Math.max(toplamKatki + devletKatkisi, toplamBirikim)
    });
  };
  ...
  const handleCopy = () => {
    if (!sonuc) return;
    const text = `BES Toplam Birikim: ${sonuc.toplamBirikim.toFixed(2)} ₺ | Devlet Katkısı: ${sonuc.devletKatkisi.toFixed(2)} ₺`;
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mb-6 font-medium">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-800 dark:text-slate-300">Bireysel Emeklilik (BES) Getiri Hesaplama</span>
      </div>

      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">Bireysel Emeklilik (BES) Getiri Hesaplama 2026 - Ücretsiz ve Hızlı Sonuçlar</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          Aylık katkı payı ve devlet katkısı (%30) avantajıyla BES birikiminizin gelecekteki tahmini değerini hesaplayın.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-4">
          <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-bold text-slate-800 dark:text-white">Katkı Bilgileri</h2>
              <button 
                onClick={handleClear}
            ...
            <div className="space-y-5">
              <div>
                <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">
                  Aylık Katkı Payı (TL) <span className="text-red-500">*</span>
                </label>
                <input 
                  type="number" 
                  step="0.01"
                  value={aylikKatki}
                  onChange={(e) => {
                    setAylikKatki(e.target.value === '' ? '' : Number(e.target.value));
                    setError(null);
                  }}
              ...
              <div>
                <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">
                  Sistemde Kalış Süresi (Yıl) <span className="text-red-500">*</span>
                </label>
                <input 
                  type="number" 
                  value={sureYil}
                  onChange={(e) => {
                    setSureYil(e.target.value === '' ? '' : Number(e.target.value));
                    setError(null);
                  }}
                  placeholder="Örn: 10" 
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-black/10 dark:border-white/10 rounded-xl px-4 py-3 text-slate-700 dark:text-white font-medium focus:outline-none focus:ring-2 focus:ring-[#0056b3]/20"
              ...
              <div>
                ...
                <input placeholder="Örn: 40" 
                  type="number" 
                  step="0.1"
                  value={beklenenGetiri}
                  onChange={(e) => {
                    setBeklenenGetiri(e.target.value === '' ? 40 : Number(e.target.value));
                    setError(null);
                  }}
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-black/10 dark:border-white/10 rounded-xl px-4 py-3 text-slate-700 dark:text-white font-medium focus:outline-none focus:ring-2 focus:ring-[#0056b3]/20"
                />
          ...
          <div className="lg:col-span-5 flex flex-col">
            <div className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-3xl p-6 md:p-8 flex-1 relative shadow-lg border border-black/5 dark:border-slate-800 flex flex-col justify-between">
              <div>
                <h3 className="text-sm font-bold uppercase tracking-widest text-slate-800 dark:text-white mb-6">Birikim Özeti</h3>

                {sonuc !== null ? (
                  <div className="space-y-4 mb-6">
                    <div>
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">Ödenen Toplam Katkı</span>
                      <div className="text-lg font-bold text-slate-800 dark:text-white">
                        {sonuc.toplamKatki.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺
                      </div>
                    </div>

                    <div>
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">Devlet Katkısı (%30)</span>
                      <div className="text-lg font-bold text-emerald-600 dark:text-emerald-400">
                        + {sonuc.devletKatkisi.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺
                      </div>
                    </div>

                    <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">Vade Sonu Toplam Birikim</span>
                      <div className="text-3xl font-black text-[#0056b3] dark:text-blue-400">
                        {sonuc.toplamBirikim.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺
                      </div>
                    </div>
                  </div>
                ) : (
                  <div className="py-12 text-center text-slate-400 text-sm">
                    Bilgileri girip hesapla butonuna tıklayın.
                  </div>
                )}
              </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm space-y-6">
        <h3 className="text-xl font-bold text-slate-800 dark:text-white flex items-center gap-2 border-b border-black/5 dark:border-white/10 pb-4">
          ...
          BES Hesaplama Rehberi
        </h3>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          Bireysel Emeklilik Sistemi'nde ödediğiniz her katkı payı için %30 oranında devlet katkısı hak edersiniz. Fon getirileri piyasa koşullarına göre değişkenlik gösterir.
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm mb-8">
        <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-6 flex items-center gap-2 border-b border-black/5 dark:border-white/10 pb-4">
          <Info className="text-[#0056b3] dark:text-blue-400 shrink-0" size={24} /> 
          Bireysel Emeklilik (BES) Getiri Hesaplama Hakkında Her Şey ve Sıkça Sorulan Sorular
        </h3>
        ...
        <div className="prose prose-sm md:prose-base max-w-none text-slate-600 dark:text-slate-400 space-y-8 leading-relaxed">
            ...
            <div className="space-y-4 bg-slate-50 dark:bg-slate-800/30 p-4 md:p-6 rounded-2xl border border-black/5 dark:border-white/5">
              <div>
                ...
                <div className="bg-white dark:bg-slate-900 border border-black/10 dark:border-white/10 px-4 py-2.5 rounded-xl font-mono text-sm text-[#0056b3] dark:text-blue-400 mb-2 overflow-x-auto">
                  BES Toplam Birikim = (Aylık Katkı Payı × Ödenen Ay Sayısı) + Devlet Katkısı (%30) + Fon Getiri Oranı
                </div>
              </div>
            </div>
          </div>
          ...
          <section className="pt-4 border-t border-black/5 dark:border-white/5">
            ...
            <div className="space-y-6">
              <div>
                <h5 className="font-bold text-slate-800 dark:text-slate-200 mb-2">Bireysel Emeklilik (BES) Getiri Hesaplama aracı ücretli midir?</h5>
      ...
      <RelatedTools category="finans" currentToolId="bes-getiri-hesaplama" />
      <div className="mt-8">
