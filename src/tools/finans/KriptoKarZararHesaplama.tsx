...
import { ChevronRight, RefreshCw, Copy, Info, Bitcoin, AlertCircle, Check } from 'lucide-react';
...
export default function KriptoKarZararHesaplama() {
  const [coinMiktari, setCoinMiktari] = useState<number | ''>('');
  const [alisFiyati, setAlisFiyati] = useState<number | ''>('');
  const [satisFiyati, setSatisFiyati] = useState<number | ''>('');
  const [komisyonOrani, setKomisyonOrani] = useState<number>(0.1);
  ...
  const handleCalculate = () => {
    if (coinMiktari === '' || alisFiyati === '' || satisFiyati === '') {
      setError('*Lütfen coin miktarı, alış fiyatı ve satış fiyatını doldurun.');
      setSonuc(null);
      return;
    }
    setError(null);

    const miktar = Number(coinMiktari);
    const alis = Number(alisFiyati);
    const satis = Number(satisFiyati);
    const komOran = Number(komisyonOrani) / 100;

    const toplamMaliyet = miktar * alis;
    const brutSatis = miktar * satis;
    const komisyonTutar = (toplamMaliyet + brutSatis) * komOran;
    const netKarZarar = brutSatis - toplamMaliyet - komisyonTutar;
    const getiriYuzdesi = (netKarZarar / toplamMaliyet) * 100;

    setSonuc({
      toplamMaliyet,
      toplamSatis: brutSatis,
      komisyonTutar,
      netKarZarar,
      getiriYuzdesi
    });
  ...
  const handleCopy = () => {
    if (!sonuc) return;
    const text = `Net Kâr/Zarar: ${sonuc.netKarZarar.toFixed(2)} $ (%${sonuc.getiriYuzdesi.toFixed(2)})`;
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mb-6 font-medium">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-800 dark:text-slate-300">Kripto Para Kâr / Zarar Hesaplama</span>
      </div>

      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">Kripto Para Kâr / Zarar Hesaplama 2026 - Ücretsiz ve Hızlı Sonuçlar</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          Kripto para alım-satım işlemlerinizde komisyon kesintileri dahil net kâr ve zarar durumunuzu hesaplayın.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-4">
          <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-bold text-slate-800 dark:text-white">İşlem Bilgileri</h2>
              <button 
                onClick={handleClear}
            ...
            <div className="space-y-5">
              <div>
                <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">
                  Coin Miktarı <span className="text-red-500">*</span>
                </label>
                <input 
                  type="number" 
                  step="any"
                  value={coinMiktari}
                  onChange={(e) => {
                    setCoinMiktari(e.target.value === '' ? '' : Number(e.target.value));
                    setError(null);
                  }}
                  placeholder="Örn: 0.5" 
              ...
              <div>
                <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">
                  Alış Fiyatı ($ veya ₺) <span className="text-red-500">*</span>
                </label>
                <input 
                  type="number" 
                  step="any"
              ...
              <div>
                <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">
                  Satış Fiyatı ($ veya ₺) <span className="text-red-500">*</span>
                </label>
                <input 
                  type="number" 
                  step="any"
                  value={satisFiyati}
                  onChange={(e) => {
                    setSatisFiyati(e.target.value === '' ? '' : Number(e.target.value));
                    setError(null);
                  }}
                  placeholder="Örn: 68000" 
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-black/10 dark:border-white/10 rounded-xl px-4 py-3 text-slate-700 dark:text-white font-medium focus:outline-none focus:ring-2 focus:ring-[#0056b3]/20"
                />
              </div>
              ...
              <div>
                ...
                <input 
                  ...
                  value={komisyonOrani}
                  onChange={(e) => {
                    setKomisyonOrani(e.target.value === '' ? 0.1 : Number(e.target.value));
                    setError(null);
                  }}
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-black/10 dark:border-white/10 rounded-xl px-4 py-3 text-slate-700 dark:text-white font-medium focus:outline-none focus:ring-2 focus:ring-[#0056b3]/20"
                />
              ...
              <div>
                ...
                {sonuc !== null ? (
                  <div className="space-y-4 mb-6">
                    <div>
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">Toplam Komisyon Kesintisi</span>
                      <div className="text-lg font-bold text-rose-600 dark:text-rose-400">
                        - {sonuc.komisyonTutar.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </div>
                    </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm space-y-6">
        <h3 className="text-xl font-bold text-slate-800 dark:text-white flex items-center gap-2 border-b border-black/5 dark:border-white/10 pb-4">
          ...
          Kripto Kâr / Zarar Hesaplama Rehberi
        </h3>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          Kripto para işlemlerinde borsaların uyguladığı maker/taker komisyonları toplam kazancınızı doğrudan etkiler. Bu araç ile net getirinizi kolayca hesaplayabilirsiniz.
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm mb-8">
        <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-6 flex items-center gap-2 border-b border-black/5 dark:border-white/10 pb-4">
          <Info className="text-[#0056b3] dark:text-blue-400 shrink-0" size={24} /> 
          Kripto Para Kâr / Zarar Hesaplama Hakkında Her Şey ve Sıkça Sorulan Sorular
        </h3>
        ...
        <div className="prose prose-sm md:prose-base max-w-none text-slate-600 dark:text-slate-400 space-y-8 leading-relaxed">
            ...
            <div className="space-y-4 bg-slate-50 dark:bg-slate-800/30 p-4 md:p-6 rounded-2xl border border-black/5 dark:border-white/5">
              <div>
                ...
                <div className="bg-white dark:bg-slate-900 border border-black/10 dark:border-white/10 px-4 py-2.5 rounded-xl font-mono text-sm text-[#0056b3] dark:text-blue-400 mb-2 overflow-x-auto">
                  Kâr/Zarar = (Satış Fiyatı × Miktar - Satış Komisyonu) - (Alış Fiyatı × Miktar + Alış Komisyonu)
                </div>
              </div>
            </div>
          </div>
          ...
          <section className="pt-4 border-t border-black/5 dark:border-white/5">
            ...
            <div className="space-y-6">
              <div>
                <h5 className="font-bold text-slate-800 dark:text-slate-200 mb-2">Kripto Para Kâr / Zarar Hesaplama aracı ücretli midir?</h5>
      ...
      <RelatedTools category="finans" currentToolId="kripto-kar-zarar-hesaplama" />
      <div className="mt-8">
