...
import { ChevronRight, RefreshCw, Copy, Info, Percent, AlertCircle, Check } from 'lucide-react';
...
export default function MevduatFaiziHesaplama() {
  ...
  const [vadeGun, setVadeGun] = useState<number | ''>('');
  const [stopajOrani, setStopajOrani] = useState<number>(7.5);
  const [sonuc, setSonuc] = useState<{
    brutFaiz: number;
    stopajKesintisi: number;
    netFaizKazanci: number;
  ...
  const handleCalculate = () => {
    if (anapara === '' || faizOrani === '' || vadeGun === '') {
      setError('*Lütfen anapara, yıllık faiz oranı ve vade (gün) alanlarını doldurun.');
      setSonuc(null);
      return;
    }
    setError(null);
    ...
    const t = Number(vadeGun);
    const stopaj = Number(stopajOrani) / 100;

    const brutFaiz = P * r * (t / 365);
    const stopajKesintisi = brutFaiz * stopaj;
    const netFaizKazanci = brutFaiz - stopajKesintisi;
    const vadeSonuToplam = P + netFaizKazanci;
  ...
  const handleCopy = () => {
    if (!sonuc) return;
    const text = `Vade Sonu Toplam: ${sonuc.vadeSonuToplam.toFixed(2)} ₺ | Net Faiz Kazancı: ${sonuc.netFaizKazanci.toFixed(2)} ₺`;
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      ...
      <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mb-6 font-medium">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-800 dark:text-slate-300">Mevduat Faizi ve Net Kazanç Hesaplama</span>
      </div>

      {/* 2. BAŞLIK VE AÇIKLAMA */}
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">Mevduat Faizi ve Net Kazanç Hesaplama 2026 - Ücretsiz ve Hızlı Sonuçlar</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          Bankaya yatırdığınız paranın vade süresince getireceği brüt faizi, stopaj kesintisini ve net kazancınızı hesaplayın.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-4">
            ...
            <div className="space-y-5">
              <div>
                <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">
                  Anapara Tutarı (TL) <span className="text-red-500">*</span>
                </label>
                <input 
                  type="number" 
                  step="0.01"
              ...
              <div>
                <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">
                  Vade Süresi (Gün) <span className="text-red-500">*</span>
                </label>
                <input 
                  type="number" 
                  value={vadeGun}
                  onChange={(e) => {
                    setVadeGun(e.target.value === '' ? '' : Number(e.target.value));
                    setError(null);
                  }}
                  placeholder="Örn: 32" 
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-black/10 dark:border-white/10 rounded-xl px-4 py-3 text-slate-700 dark:text-white font-medium focus:bg-white dark:focus:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-[#0056b3]/20 dark:focus:ring-blue-500/30 focus:border-[#0056b3] dark:focus:border-blue-500 transition-all"
              ...
              <div>
                ...
                <input 
                  ...
                  value={stopajOrani}
                  onChange={(e) => {
                    setStopajOrani(e.target.value === '' ? 7.5 : Number(e.target.value));
                    setError(null);
                  }}
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-black/10 dark:border-white/10 rounded-xl px-4 py-3 text-slate-700 dark:text-white font-medium focus:bg-white dark:focus:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-[#0056b3]/20 dark:focus:ring-blue-500/30 focus:border-[#0056b3] dark:focus:border-blue-500 transition-all"
                />
          ...
          <div className="lg:col-span-5 flex flex-col">
            <div className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-3xl p-6 md:p-8 flex-1 relative overflow-hidden shadow-lg border border-black/5 dark:border-slate-800 flex flex-col justify-between">
              <div>
                <h3 className="text-sm font-bold uppercase tracking-widest text-slate-800 dark:text-white mb-6">Mevduat Özeti</h3>

                {sonuc !== null ? (
                  <div className="space-y-4 mb-6">
                    <div>
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">Brüt Faiz Kazancı</span>
                      <div className="text-lg font-bold text-slate-800 dark:text-white">
                        {sonuc.brutFaiz.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺
                      </div>
                    </div>

                    <div>
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">Stopaj Kesintisi (%{stopajOrani})</span>
                      <div className="text-lg font-bold text-rose-600 dark:text-rose-400">
                        - {sonuc.stopajKesintisi.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺
                      </div>
                    </div>

                    <div>
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">Net Faiz Kazancı</span>
                      <div className="text-xl font-bold text-emerald-600 dark:text-emerald-400">
                        + {sonuc.netFaizKazanci.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺
                      </div>
                    </div>

                    <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">Vade Sonu Toplam Tutar</span>
                      <div className="text-3xl font-black text-[#0056b3] dark:text-blue-400">
                        {sonuc.vadeSonuToplam.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺
                      </div>
                    </div>
                  </div>
                ) : (
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm space-y-6">
        ...
        <div className="prose prose-sm md:prose-base max-w-none text-slate-600 dark:text-slate-400 space-y-6 leading-relaxed">
          <div>
            <h4 className="text-[17px] font-bold text-slate-800 dark:text-slate-200 mb-2">Mevduat Faizi Nasıl Hesaplanır?</h4>
            <p>
              Mevduat kazancı, anaparanın yıllık faiz oranı ve vade gününe göre 365 güne bölünmesiyle brüt olarak hesaplanır. Elde edilen brüt tutardan yasal stopaj vergisi düşülerek net kazanca ulaşılır.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm mb-8">
        <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-6 flex items-center gap-2 border-b border-black/5 dark:border-white/10 pb-4">
          <Info className="text-[#0056b3] dark:text-blue-400 shrink-0" size={24} /> 
          Mevduat Faizi ve Net Kazanç Hesaplama Hakkında Her Şey ve Sıkça Sorulan Sorular
        </h3>
        ...
        <div className="prose prose-sm md:prose-base max-w-none text-slate-600 dark:text-slate-400 space-y-8 leading-relaxed">
            ...
            <div className="space-y-4 bg-slate-50 dark:bg-slate-800/30 p-4 md:p-6 rounded-2xl border border-black/5 dark:border-white/5">
              <div>
                ...
                <div className="bg-white dark:bg-slate-900 border border-black/10 dark:border-white/10 px-4 py-2.5 rounded-xl font-mono text-sm text-[#0056b3] dark:text-blue-400 mb-2 overflow-x-auto">
                  Brüt Getiri = (Anapara × Faiz Oranı × Vade Gün Sayısı) / 36500 | Net Getiri = Brüt Getiri × (1 - Stopaj Oranı)
                </div>
              </div>
            </div>
          </div>
          ...
          <section className="pt-4 border-t border-black/5 dark:border-white/5">
            ...
            <div className="space-y-6">
              <div>
                <h5 className="font-bold text-slate-800 dark:text-slate-200 mb-2">Mevduat Faizi ve Net Kazanç Hesaplama aracı ücretli midir?</h5>
      ...
      <RelatedTools category="finans" currentToolId="mevduat-faizi-hesaplama" />
      <div className="mt-8">
