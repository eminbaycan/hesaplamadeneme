...
import { ChevronRight, RefreshCw, Copy, Info, DollarSign, AlertCircle, Check } from 'lucide-react';
...
export default function DovizMaliyetiHesaplama() {
  const [dovizMiktari, setDovizMiktari] = useState<number | ''>('');
  const [alisKuru, setAlisKuru] = useState<number | ''>('');
  const [guncelKur, setGuncelKur] = useState<number | ''>('');
  const [sonuc, setSonuc] = useState<{
    toplamMaliyetTL: number;
    guncelDegerTL: number;
    netKarZararTL: number;
  ...
  const handleCalculate = () => {
    if (dovizMiktari === '' || alisKuru === '' || guncelKur === '') {
      setError('*Lütfen döviz miktarı, alış kuru ve güncel kur alanlarını doldurun.');
      setSonuc(null);
      return;
    }
    setError(null);

    const miktar = Number(dovizMiktari);
    const kurAlis = Number(alisKuru);
    const kurGuncel = Number(guncelKur);

    const toplamMaliyetTL = miktar * kurAlis;
    const guncelDegerTL = miktar * kurGuncel;
    const netKarZararTL = guncelDegerTL - toplamMaliyetTL;
    const getiriYuzdesi = (netKarZararTL / toplamMaliyetTL) * 100;
  ...
  const handleCopy = () => {
    if (!sonuc) return;
    const text = `Toplam Maliyet: ${sonuc.toplamMaliyetTL.toFixed(2)} ₺ | Net Kâr/Zarar: ${sonuc.netKarZararTL.toFixed(2)} ₺ (%${sonuc.getiriYuzdesi.toFixed(2)})`;
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      ...
      <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mb-6 font-medium">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-800 dark:text-slate-300">Döviz Maliyeti ve Kur Farkı Hesaplama</span>
      </div>

      {/* 2. BAŞLIK VE AÇIKLAMA */}
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">Döviz Maliyeti ve Kur Farkı Hesaplama 2026 - Ücretsiz ve Hızlı Sonuçlar</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          Aldığınız döviz miktarı, alış kuru ve güncel kur değerleri üzerinden toplam maliyetinizi, TL karşılığını ve kur farkı kâr/zarar durumunuzu hesaplayın.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-4">
            ...
            <div className="space-y-5">
              <div>
                <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">
                  Döviz Miktarı (Örn: $ veya €) <span className="text-red-500">*</span>
                </label>
                <input 
                  type="number" 
                  step="0.01"
                  value={dovizMiktari}
                  onChange={(e) => {
                    setDovizMiktari(e.target.value === '' ? '' : Number(e.target.value));
                    setError(null);
                  }}
              ...
              <div>
                <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">
                  Alış Kuru (TL) <span className="text-red-500">*</span>
                </label>
                <input 
                  type="number" 
                  step="0.0001"
                  value={alisKuru}
                  onChange={(e) => {
                    setAlisKuru(e.target.value === '' ? '' : Number(e.target.value));
                    setError(null);
                  }}
              ...
              <div>
                <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">
                  Güncel / Satış Kuru (TL) <span className="text-red-500">*</span>
                </label>
                <input 
                  type="number" 
                  step="0.0001"
                  value={guncelKur}
                  onChange={(e) => {
                    setGuncelKur(e.target.value === '' ? '' : Number(e.target.value));
                    setError(null);
                  }}
          ...
          <div className="lg:col-span-5 flex flex-col">
            <div className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-3xl p-6 md:p-8 flex-1 relative overflow-hidden shadow-lg border border-black/5 dark:border-slate-800 flex flex-col justify-between">
              <div>
                <h3 className="text-sm font-bold uppercase tracking-widest text-slate-800 dark:text-white mb-6">Yatırım Özeti</h3>

                {sonuc !== null ? (
                  <div className="space-y-4 mb-6">
                    <div>
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">Toplam Alış Maliyeti (TL)</span>
                      <div className="text-xl font-bold text-slate-800 dark:text-white">
                        {sonuc.toplamMaliyetTL.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺
                      </div>
                    </div>

                    <div>
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">Güncel Değer (TL)</span>
                      <div className="text-xl font-bold text-slate-800 dark:text-white">
                        {sonuc.guncelDegerTL.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺
                      </div>
                    </div>

                    <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">Kur Farkı Kâr / Zarar</span>
                      <div className={`text-2xl font-black ${sonuc.netKarZararTL >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
                        {sonuc.netKarZararTL >= 0 ? '+' : ''}{sonuc.netKarZararTL.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺ 
                  ...
                  <div className="py-12 text-center text-slate-400 text-sm">
                    Döviz bilgilerini girip hesapla butonuna tıklayın.
                  </div>
                )}
              </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm space-y-6">
        <h3 className="text-xl font-bold text-slate-800 dark:text-white flex items-center gap-2 border-b border-black/5 dark:border-white/10 pb-4">
          ...
          Döviz Maliyeti ve Kur Farkı Hesaplama Rehberi
        </h3>

        <div className="prose prose-sm md:prose-base max-w-none text-slate-600 dark:text-slate-400 space-y-6 leading-relaxed">
          <div>
            <h4 className="text-[17px] font-bold text-slate-800 dark:text-slate-200 mb-2">Kur Farkı ve Döviz Maliyeti Nasıl Hesaplanır?</h4>
            <p>
              Döviz yatırımlarında maliyet, alınan döviz miktarının alış kuruyla çarpılmasıyla bulunur. Güncel kurdaki değişimler sonucunda elde edilen TL değeri ile başlangıç maliyeti arasındaki fark kur farkı kâr ya da zararını verir.
            </p>
          </div>
          ...
          <div className="pt-4 border-t border-black/5 dark:border-white/5">
            ...
            <div className="space-y-4 bg-slate-50 dark:bg-slate-800/30 p-4 md:p-6 rounded-2xl border border-black/5 dark:border-white/5">
              <div>
                <div className="font-semibold text-slate-800 dark:text-slate-200 mb-1">Kur Farkı Formülü:</div>
                <div className="bg-white dark:bg-slate-900 border border-black/10 dark:border-white/10 px-4 py-2.5 rounded-xl font-mono text-sm text-[#0056b3] dark:text-blue-400 mb-2 overflow-x-auto">
                  Toplam Maliyet = Döviz Miktarı × Alış Kuru <br />
                  Güncel Değer = Döviz Miktarı × Güncel Kur <br />
                  Kur Farkı = Güncel Değer - Toplam Maliyet
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm mb-8">
        <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-6 flex items-center gap-2 border-b border-black/5 dark:border-white/10 pb-4">
          <Info className="text-[#0056b3] dark:text-blue-400 shrink-0" size={24} /> 
          Döviz Maliyeti ve Kur Farkı Hesaplama Hakkında Her Şey ve Sıkça Sorulan Sorular
        </h3>
        ...
        <div className="prose prose-sm md:prose-base max-w-none text-slate-600 dark:text-slate-400 space-y-8 leading-relaxed">
          ...
          <section className="pt-4 border-t border-black/5 dark:border-white/5">
            ...
            <div className="space-y-6">
              <div>
                <h5 className="font-bold text-slate-800 dark:text-slate-200 mb-2">Döviz Maliyeti ve Kur Farkı Hesaplama aracı ücretli midir?</h5>
      ...
      <RelatedTools category="finans" currentToolId="doviz-maliyeti-hesaplama" />
