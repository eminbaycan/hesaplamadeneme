...
import { ChevronRight, RefreshCw, Copy, Info, FileText, AlertCircle, Check } from 'lucide-react';
...
export default function SerbestMeslekMakbuzuHesaplama() {
  const [brutTutar, setBrutTutar] = useState<number | ''>('');
  const [stopajOrani, setStopajOrani] = useState<number>(20);
  ...
  const handleCalculate = () => {
    if (brutTutar === '') {
      setError('*Lütfen brüt tutar alanını doldurun.');
      setSonuc(null);
      return;
    }
    setError(null);
    ...
    // Net ele geçen = Brüt - Stopaj + KDV (KDV dahil fatura kesiliyorsa veya net tahsilat)
    const netTahsilat = brut - stopajTutar + kdvTutar;
  ...
  const handleCopy = () => {
    if (!sonuc) return;
    const text = `Brüt: ${sonuc.brut.toFixed(2)} ₺ | Net Tahsilat: ${sonuc.netTahsilat.toFixed(2)} ₺`;
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mb-6 font-medium">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-800 dark:text-slate-300">Serbest Meslek Makbuzu (SMM) Hesaplama</span>
      </div>

      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">Serbest Meslek Makbuzu Hesaplama 2026 - Ücretsiz ve Hızlı Sonuçlar</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          Brüt hizmet bedeli üzerinden stopaj, KDV ve net tahsilat tutarınızı anında hesaplayın.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-4">
          <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-bold text-slate-800 dark:text-white">Makbuz Bilgileri</h2>
              <button 
                onClick={handleClear}
            ...
            <div className="space-y-5">
              <div>
                <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">
                  Brüt Tutar (TL) <span className="text-red-500">*</span>
                </label>
                <input 
                  type="number" 
                  step="0.01"
                  value={brutTutar}
                  onChange={(e) => {
                    setBrutTutar(e.target.value === '' ? '' : Number(e.target.value));
                    setError(null);
                  }}
              ...
              <div>
                ...
                >
                  ...
                  <option value={15}>%15 (Bazı Serbest Meslekler)</option>
                </select>
              </div>
          ...
          <div className="lg:col-span-5 flex flex-col">
            <div className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-3xl p-6 md:p-8 flex-1 relative shadow-lg border border-black/5 dark:border-slate-800 flex flex-col justify-between">
              <div>
                <h3 className="text-sm font-bold uppercase tracking-widest text-slate-800 dark:text-white mb-6">Makbuz Özeti</h3>

                {sonuc !== null ? (
                  <div className="space-y-4 mb-6">
                    <div>
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">Stopaj Kesintisi (-</span>
                      <div className="text-lg font-bold text-rose-600 dark:text-rose-400">
                        {sonuc.stopajTutar.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺
                      </div>
                    </div>

                    <div>
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">KDV Tutarı (+</span>
                    ...
                    <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">Net Tahsilat Tutarı</span>
                      <div className="text-3xl font-black text-[#0056b3] dark:text-blue-400">
                        {sonuc.netTahsilat.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺
                      </div>
                    </div>
                  </div>
                ) : (
                  <div className="py-12 text-center text-slate-400 text-sm">
                    Brüt tutarı girip hesapla butonuna tıklayın.
                  </div>
                )}
              </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm space-y-6">
        <h3 className="text-xl font-bold text-slate-800 dark:text-white flex items-center gap-2 border-b border-black/5 dark:border-white/10 pb-4">
          ...
          SMM Hesaplama Rehberi
        </h3>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          Serbest meslek erbabı olarak düzenlenen makbuzlarda brüt tutardan gelir vergisi stopajı kesilir ve hizmet bedeline KDV eklenir.
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm mb-8">
        <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-6 flex items-center gap-2 border-b border-black/5 dark:border-white/10 pb-4">
          <Info className="text-[#0056b3] dark:text-blue-400 shrink-0" size={24} /> 
          Serbest Meslek Makbuzu Hesaplama Hakkında Her Şey ve Sıkça Sorulan Sorular
        </h3>
        ...
        <div className="prose prose-sm md:prose-base max-w-none text-slate-600 dark:text-slate-400 space-y-8 leading-relaxed">
            ...
            <div className="space-y-4 bg-slate-50 dark:bg-slate-800/30 p-4 md:p-6 rounded-2xl border border-black/5 dark:border-white/5">
              <div>
                ...
                <div className="bg-white dark:bg-slate-900 border border-black/10 dark:border-white/10 px-4 py-2.5 rounded-xl font-mono text-sm text-[#0056b3] dark:text-blue-400 mb-2 overflow-x-auto">
                  Brüt Ücret = Net Ücret / 0.80 | Stopaj = Brüt × %20 | KDV = Brüt × %20 | Net Tahsilat = Brüt - Stopaj + KDV
                </div>
              </div>
            </div>
          </div>
          ...
          <section className="pt-4 border-t border-black/5 dark:border-white/5">
            ...
            <div className="space-y-6">
              <div>
                <h5 className="font-bold text-slate-800 dark:text-slate-200 mb-2">Serbest Meslek Makbuzu Hesaplama aracı ücretli midir?</h5>
      ...
      <RelatedTools category="finans" currentToolId="serbest-meslek-makbuzu-hesaplama" />
      <div className="mt-8">
