...
export default function BilesikFaizGetirisiHesaplama() {
  const [anappara, setAnappara] = useState<number | ''>('');
  const [faizOrani, setFaizOrani] = useState<number | ''>('');
  const [vadeYil, setVadeYil] = useState<number | ''>('');
  ...
  const handleCalculate = () => {
    if (anappara === '' || faizOrani === '' || vadeYil === '') {
      setError('*Lütfen anappara, yıllık faiz oranı ve vade (yıl) alanlarını doldurun.');
      setSonuc(null);
      return;
    }
    setError(null);

    const P = Number(anappara);
    const r = Number(faizOrani) / 100;
    const t = Number(vadeYil);

    const toplamTutar = P * Math.pow(1 + r, t);
    const netKazanc = toplamTutar - P;
  ...
  const handleCopy = () => {
    if (!sonuc) return;
    const text = `Toplam Vade Sonu Tutar: ${sonuc.toplamTutar.toFixed(2)} ₺ | Net Kazanç: ${sonuc.netKazanc.toFixed(2)} ₺`;
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      ...
      <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mb-6 font-medium">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-800 dark:text-slate-300">Bileşik Faiz Getirisi Hesaplama</span>
      </div>

      {/* 2. BAŞLIK VE AÇIKLAMA */}
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">Bileşik Faiz Getirisi Hesaplama 2026 - Ücretsiz ve Hızlı Sonuçlar</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          Anaparanızın yıllık bileşik faiz oranı ve vade süresi boyunca elde edeceği toplam birikimi ve faiz kazancını hesaplayın.
        </p>
      </div>
            ...
            <div className="space-y-5">
              <div>
                <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">
                  Anapara / Başlangıç Tutarı (TL) <span className="text-red-500">*</span>
                </label>
                <input 
                  type="number" 
                  step="0.01"
                  value={anappara}
                  onChange={(e) => {
                    setAnappara(e.target.value === '' ? '' : Number(e.target.value));
                    setError(null);
                  }}
              ...
              <div>
                <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">
                  Vade Süresi (Yıl) <span className="text-red-500">*</span>
                </label>
                <input 
                  type="number" 
                  value={vadeYil}
                  onChange={(e) => {
                    setVadeYil(e.target.value === '' ? '' : Number(e.target.value));
                    setError(null);
                  }}
                  placeholder="Örn: 3" 
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-black/10 dark:border-white/10 rounded-xl px-4 py-3 text-slate-700 dark:text-white font-medium focus:bg-white dark:focus:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-[#0056b3]/20 dark:focus:ring-blue-500/30 focus:border-[#0056b3] dark:focus:border-blue-500 transition-all"
                  ...
                  <div className="space-y-4 mb-6">
                    <div>
                      ...
                      <div className="text-xl font-bold text-emerald-600 dark:text-emerald-400">
                        + {sonuc.netKazanc.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺
                      </div>
                    </div>
                    ...
                    <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
                      ...
                      <div className="text-3xl font-black text-[#0056b3] dark:text-blue-400">
                        {sonuc.toplamTutar.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺
                      </div>
                    </div>
                  </div>
                ) : (
                  <div className="py-12 text-center text-slate-400 text-sm">
                    Yatırım detaylarını girip hesapla butonuna tıklayın.
                  </div>
                )}
              </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm space-y-6">
        <h3 className="text-xl font-bold text-slate-800 dark:text-white flex items-center gap-2 border-b border-black/5 dark:border-white/10 pb-4">
          ...
          Bileşik Faiz Hesaplama Rehberi
        </h3>

        <div className="prose prose-sm md:prose-base max-w-none text-slate-600 dark:text-slate-400 space-y-6 leading-relaxed">
          <div>
            <h4 className="text-[17px] font-bold text-slate-800 dark:text-slate-200 mb-2">Bileşik Faiz Nedir?</h4>
            <p>
              Bileşik faiz, anaparanın yanı sıra önceki dönemlerde biriken faizlerin de faiz kazanmaya devam ettiği bir finansal büyüme modelidir.
            </p>
          </div>
            ...
            <div className="space-y-4 bg-slate-50 dark:bg-slate-800/30 p-4 md:p-6 rounded-2xl border border-black/5 dark:border-white/5">
              <div>
                ...
                <div className="bg-white dark:bg-slate-900 border border-black/10 dark:border-white/10 px-4 py-2.5 rounded-xl font-mono text-sm text-[#0056b3] dark:text-blue-400 mb-2 overflow-x-auto">
                  Toplam Tutar = Anapara × (1 + Faiz Oranı) ^ Vade (Yıl)
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm mb-8">
        <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-6 flex items-center gap-2 border-b border-black/5 dark:border-white/10 pb-4">
          <Info className="text-[#0056b3] dark:text-blue-400 shrink-0" size={24} /> 
          Bileşik Faiz Getirisi Hesaplama Hakkında Her Şey ve Sıkça Sorulan Sorular
        </h3>
        ...
        <div className="prose prose-sm md:prose-base max-w-none text-slate-600 dark:text-slate-400 space-y-8 leading-relaxed">
          ...
          <section className="pt-4 border-t border-black/5 dark:border-white/5">
            ...
            <div className="space-y-6">
              <div>
                <h5 className="font-bold text-slate-800 dark:text-slate-200 mb-2">Bileşik Faiz Getirisi Hesaplama aracı ücretli midir?</h5>
      ...
      <RelatedTools category="finans" currentToolId="bilesik-faiz-hesaplama" />
