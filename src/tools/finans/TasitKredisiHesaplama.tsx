...
export default function TasitKredisiHesaplama() {
  const [krediTutari, setKrediTutari] = useState<number | ''>('');
  const [faizOrani, setFaizOrani] = useState<number | ''>('');
  const [vadeAy, setVadeAy] = useState<number | ''>('');
  ...
  const handleCalculate = () => {
    if (krediTutari === '' || faizOrani === '' || vadeAy === '') {
      setError('*Lütfen kredi tutarı, aylık faiz oranı ve vade alanlarını doldurun.');
      setSonuc(null);
      return;
    }
    setError(null);

    const P = Number(krediTutari);
    const r = Number(faizOrani) / 100;
    const n = Number(vadeAy);

    if (r === 0) {
      const aylikTaksit = P / n;
      setSonuc({
        aylikTaksit,
        toplamOdeme: P,
        toplamFaiz: 0
    ...
    const toplamFaiz = toplamOdeme - P;
  ...
  const handleCopy = () => {
    if (!sonuc) return;
    const text = `Aylık Taksit: ${sonuc.aylikTaksit.toFixed(2)} ₺ | Toplam Ödeme: ${sonuc.toplamOdeme.toFixed(2)} ₺`;
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mb-6 font-medium">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-800 dark:text-slate-300">Taşıt Kredisi Hesaplama</span>
      </div>

      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">Taşıt Kredisi Hesaplama 2026 - Ücretsiz ve Hızlı Sonuçlar</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          Araç alımlarınız için çektiğiniz taşıt kredisinin aylık taksit tutarını ve toplam geri ödemesini hesaplayın.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-4">
          <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-bold text-slate-800 dark:text-white">Kredi Bilgileri</h2>
              <button 
                onClick={handleClear}
            ...
            <div className="space-y-5">
              <div>
                ...
                <input 
                  ...
                  value={krediTutari}
                  onChange={(e) => {
                    setKrediTutari(e.target.value === '' ? '' : Number(e.target.value));
                    setError(null);
                  }}
          ...
          <div className="lg:col-span-5 flex flex-col">
            <div className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-3xl p-6 md:p-8 flex-1 relative shadow-lg border border-black/5 dark:border-slate-800 flex flex-col justify-between">
              <div>
                <h3 className="text-sm font-bold uppercase tracking-widest text-slate-800 dark:text-white mb-6">Ödeme Planı Özeti</h3>

                {sonuc !== null ? (
                  <div className="space-y-4 mb-6">
                    <div>
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">Aylık Taksit Tutarı</span>
                      <div className="text-2xl font-black text-[#0056b3] dark:text-blue-400">
                        {sonuc.aylikTaksit.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺
                      </div>
                    </div>

                    <div>
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">Toplam Faiz Ödemesi</span>
                      <div className="text-lg font-bold text-rose-600 dark:text-rose-400">
                        {sonuc.toplamFaiz.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺
                      </div>
                    </div>

                    <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">Toplam Geri Ödeme</span>
                      <div className="text-lg font-bold text-slate-800 dark:text-white">
                        {sonuc.toplamOdeme.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺
                      </div>
                    </div>
                  </div>
                ) : (
                  <div className="py-12 text-center text-slate-400 text-sm">
                    Kredi bilgilerini girip hesapla butonuna tıklayın.
                  </div>
                )}
              </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm space-y-6">
        ...
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          Türkiye'de taşıt kredilerinde vade sınırları aracın fatura değerine göre 12 aydan 48 aya kadar değişiklik göstermektedir.
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm mb-8">
        <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-6 flex items-center gap-2 border-b border-black/5 dark:border-white/10 pb-4">
          <Info className="text-[#0056b3] dark:text-blue-400 shrink-0" size={24} /> 
          Taşıt Kredisi Hesaplama Hakkında Her Şey ve Sıkça Sorulan Sorular
        </h3>
        ...
        <div className="prose prose-sm md:prose-base max-w-none text-slate-600 dark:text-slate-400 space-y-8 leading-relaxed">
            ...
            <div className="space-y-4 bg-slate-50 dark:bg-slate-800/30 p-4 md:p-6 rounded-2xl border border-black/5 dark:border-white/5">
              <div>
                ...
                <div className="bg-white dark:bg-slate-900 border border-black/10 dark:border-white/10 px-4 py-2.5 rounded-xl font-mono text-sm text-[#0056b3] dark:text-blue-400 mb-2 overflow-x-auto">
                  Aylık Taksit = Kredi Tutarı × [Faiz × (1 + Faiz)^Vade] / [(1 + Faiz)^Vade - 1] (Faiz oranına BSMV ve KKDF dahildir)
                </div>
              </div>
            </div>
          </div>
          ...
          <section className="pt-4 border-t border-black/5 dark:border-white/5">
            ...
            <div className="space-y-6">
              <div>
                <h5 className="font-bold text-slate-800 dark:text-slate-200 mb-2">Taşıt Kredisi Hesaplama aracı ücretli midir?</h5>
      ...
      <RelatedTools category="finans" currentToolId="tasit-kredisi-hesaplama" />
      <div className="mt-8">
