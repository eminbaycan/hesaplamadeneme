...
import { ChevronRight, RefreshCw, Copy, Info, Calendar, AlertCircle, Check } from 'lucide-react';
...
export default function YillikIzinUcretiHesaplama() {
  const [brutMaas, setBrutMaas] = useState<number | ''>('');
  const [kullanilmayanGun, setKullanilmayanGun] = useState<number | ''>('');
  const [sonuc, setSonuc] = useState<{
    brutIzinUcreti: number;
    netIzinUcreti: number;
    sgkKesintisi: number;
    vergiKesintisi: number;
  } | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [copied, setCopied] = useState(false);

  const handleCalculate = () => {
    if (brutMaas === '' || kullanilmayanGun === '') {
      setError('*Lütfen brüt maaş ve kullanılmayan izin günü alanlarını doldurun.');
      setSonuc(null);
      return;
    }
    setError(null);

    const maas = Number(brutMaas);
    const gun = Number(kullanilmayanGun);

    // Günlük brüt maaş = Brüt maaş / 30
    const gunlukBrut = maas / 30;
    const brutIzinUcreti = gunlukBrut * gun;

    // Yaklaşık yasal kesintiler (%15 SGK İşçi Payı + %15 Gelir/Damap Vergisi ortalama)
    const sgkKesintisi = brutIzinUcreti * 0.15;
    const vergiKesintisi = brutIzinUcreti * 0.15;
    const netIzinUcreti = brutIzinUcreti - sgkKesintisi - vergiKesintisi;
  ...
  const handleCopy = () => {
    if (!sonuc) return;
    const text = `Net Yıllık İzin Ücreti: ${sonuc.netIzinUcreti.toFixed(2)} ₺ | Brüt: ${sonuc.brutIzinUcreti.toFixed(2)} ₺`;
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mb-6 font-medium">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-800 dark:text-slate-300">Yıllık İzin Ücreti Hesaplama</span>
      </div>

      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">Yıllık İzin Ücreti Hesaplama 2026 - Ücretsiz ve Hızlı Sonuçlar</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          İşten ayrılma durumunda kullanılmayan yıllık izin günleri karşılığında alacağınız brüt ve net ücreti hesaplayın.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-4">
          <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-bold text-slate-800 dark:text-white">Maaş ve İzin Bilgileri</h2>
              <button 
                onClick={handleClear}
            ...
            <div className="space-y-5">
              <div>
                <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">
                  Son Brüt Maaş (TL) <span className="text-red-500">*</span>
                </label>
                <input 
                  type="number" 
                  step="0.01"
                  value={brutMaas}
                  onChange={(e) => {
                    setBrutMaas(e.target.value === '' ? '' : Number(e.target.value));
                    setError(null);
                  }}
              ...
              <div>
                <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">
                  Kullanılmayan İzin Süresi (Gün) <span className="text-red-500">*</span>
                </label>
                <input 
                  type="number" 
                  value={kullanilmayanGun}
                  onChange={(e) => {
                    setKullanilmayanGun(e.target.value === '' ? '' : Number(e.target.value));
                    setError(null);
                  }}
                  placeholder="Örn: 14" 
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-black/10 dark:border-white/10 rounded-xl px-4 py-3 text-slate-700 dark:text-white font-medium focus:outline-none focus:ring-2 focus:ring-[#0056b3]/20"
          ...
          <div className="lg:col-span-5 flex flex-col">
            <div className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-3xl p-6 md:p-8 flex-1 relative shadow-lg border border-black/5 dark:border-slate-800 flex flex-col justify-between">
              <div>
                <h3 className="text-sm font-bold uppercase tracking-widest text-slate-800 dark:text-white mb-6">İzin Ücreti Özeti</h3>

                {sonuc !== null ? (
                  <div className="space-y-4 mb-6">
                    <div>
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">Brüt İzin Alacağı</span>
                      <div className="text-lg font-bold text-slate-800 dark:text-white">
                        {sonuc.brutIzinUcreti.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺
                      </div>
                    </div>

                    <div>
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">Yasal Kesintiler (SGK + Vergi)</span>
                      <div className="text-lg font-bold text-rose-600 dark:text-rose-400">
                        - {(sonuc.sgkKesintisi + sonuc.vergiKesintisi).toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺
                      </div>
                    </div>

                    <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">Net Ödenecek İzin Ücreti</span>
                      <div className="text-3xl font-black text-[#0056b3] dark:text-blue-400">
                        {sonuc.netIzinUcreti.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺
                      </div>
                    </div>
                  </div>
                ) : (
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm space-y-6">
        <h3 className="text-xl font-bold text-slate-800 dark:text-white flex items-center gap-2 border-b border-black/5 dark:border-white/10 pb-4">
          ...
          Yıllık İzin Ücreti Rehberi
        </h3>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          İş sözleşmesinin herhangi bir nedenle sona ermesi halinde, işçinin kullandırılmayan yıllık izin sürelerine ait ücreti son brüt ücreti üzerinden ödenir.
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm mb-8">
        <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-6 flex items-center gap-2 border-b border-black/5 dark:border-white/10 pb-4">
          <Info className="text-[#0056b3] dark:text-blue-400 shrink-0" size={24} /> 
          Yıllık İzin Ücreti Hesaplama Hakkında Her Şey ve Sıkça Sorulan Sorular
        </h3>
        ...
        <div className="prose prose-sm md:prose-base max-w-none text-slate-600 dark:text-slate-400 space-y-8 leading-relaxed">
            ...
            <div className="space-y-4 bg-slate-50 dark:bg-slate-800/30 p-4 md:p-6 rounded-2xl border border-black/5 dark:border-white/5">
              <div>
                ...
                <div className="bg-white dark:bg-slate-900 border border-black/10 dark:border-white/10 px-4 py-2.5 rounded-xl font-mono text-sm text-[#0056b3] dark:text-blue-400 mb-2 overflow-x-auto">
                  İzin Ücreti = (Günlük Brüt Ücret × Kullanılmayan İzin Günü) - (SGK Payı + Gelir Vergisi + Damga Vergisi)
                </div>
              </div>
            </div>
          </div>
          ...
          <section className="pt-4 border-t border-black/5 dark:border-white/5">
            ...
            <div className="space-y-6">
              <div>
                <h5 className="font-bold text-slate-800 dark:text-slate-200 mb-2">Yıllık İzin Ücreti Hesaplama aracı ücretli midir?</h5>
      ...
      <RelatedTools category="finans" currentToolId="yillik-izin-ucreti-hesaplama" />
      <div className="mt-8">
