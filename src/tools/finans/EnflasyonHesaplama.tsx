...
import { ChevronRight, RefreshCw, Copy, Info, TrendingDown, AlertCircle, Check } from 'lucide-react';
...
export default function EnflasyonHesaplama() {
  ...
  const [enflasyonOrani, setEnflasyonOrani] = useState<number | ''>('');
  const [sureYil, setSureYil] = useState<number | ''>('');
  const [sonuc, setSonuc] = useState<{
    gelecektekiDeger: number;
    degerKaybiYuzdesi: number;
    satinAlmaGucu: number;
  } | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [copied, setCopied] = useState(false);

  const handleCalculate = () => {
    if (baslangicTutar === '' || enflasyonOrani === '' || sureYil === '') {
    ...
    const P = Number(baslangicTutar);
    const i = Number(enflasyonOrani) / 100;
    const n = Number(sureYil);

    const gelecektekiDeger = P * Math.pow(1 + i, n);
    const satinAlmaGucu = P / Math.pow(1 + i, n);
    const degerKaybiYuzdesi = ((P - satinAlmaGucu) / P) * 100;
  ...
  const handleCopy = () => {
    if (!sonuc) return;
    const text = `Gelecekteki Maliyet: ${sonuc.gelecektekiDeger.toFixed(2)} ₺ | Satın Alma Gücü: ${sonuc.satinAlmaGucu.toFixed(2)} ₺`;
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      ...
      <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mb-6 font-medium">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-800 dark:text-slate-300">Enflasyon ve Satın Alma Gücü Hesaplama</span>
      </div>

      {/* 2. BAŞLIK VE AÇIKLAMA */}
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">Enflasyon ve Satın Alma Gücü Hesaplama 2026 - Ücretsiz ve Hızlı Sonuçlar</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          Yıllık ortalama enflasyon oranına göre paranızın gelecekteki erime miktarını ve satın alma gücündeki değişimi hesaplayın.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-4">
            ...
            <div className="space-y-5">
              <div>
                <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">
                  Mevcut Tutar / Fiyat (TL) <span className="text-red-500">*</span>
                </label>
                <input 
                  type="number" 
                  step="0.01"
              ...
              <div>
                <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">
                  Yıllık Enflasyon Oranı (%) <span className="text-red-500">*</span>
                </label>
                <input 
                  type="number" 
                  step="0.01"
                  ...
                  onChange={(e) => {
                    setEnflasyonOrani(e.target.value === '' ? '' : Number(e.target.value));
                    setError(null);
                  }}
                  placeholder="Örn: 40" 
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-black/10 dark:border-white/10 rounded-xl px-4 py-3 text-slate-700 dark:text-white font-medium focus:bg-white dark:focus:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-[#0056b3]/20 dark:focus:ring-blue-500/30 focus:border-[#0056b3] dark:focus:border-blue-500 transition-all"
          ...
          <div className="lg:col-span-5 flex flex-col">
            <div className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-3xl p-6 md:p-8 flex-1 relative overflow-hidden shadow-lg border border-black/5 dark:border-slate-800 flex flex-col justify-between">
              <div>
                <h3 className="text-sm font-bold uppercase tracking-widest text-slate-800 dark:text-white mb-6">Analiz Özeti</h3>

                {sonuc !== null ? (
                  <div className="space-y-4 mb-6">
                    <div>
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">Gelecekteki Eşit Maliyet</span>
                      <div className="text-xl font-bold text-slate-800 dark:text-white">
                        {sonuc.gelecektekiDeger.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺
                      </div>
                    </div>

                    <div>
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">Paranın Bugünkü Satın Alma Gücü</span>
                      <div className="text-xl font-bold text-[#0056b3] dark:text-blue-400">
                        {sonuc.satinAlmaGucu.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺
                      </div>
                    </div>
                  </div>
                ) : (
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm space-y-6">
        <h3 className="text-xl font-bold text-slate-800 dark:text-white flex items-center gap-2 border-b border-black/5 dark:border-white/10 pb-4">
          ...
          Enflasyon ve Satın Alma Gücü Hesaplama Rehberi
        </h3>

        <div className="prose prose-sm md:prose-base max-w-none text-slate-600 dark:text-slate-400 space-y-6 leading-relaxed">
          <div>
            <h4 className="text-[17px] font-bold text-slate-800 dark:text-slate-200 mb-2">Enflasyon Paranın Değerini Nasıl Etkiler?</h4>
            <p>
              Enflasyon, mal ve hizmet fiyatlarının genel düzeyindeki artış nedeniyle paranın satın alma gücünün azalmasıdır. Yüksek enflasyon ortamında birikimlerin korunması için en az enflasyon oranında getiri sağlanması gerekir.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm mb-8">
        <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-6 flex items-center gap-2 border-b border-black/5 dark:border-white/10 pb-4">
          <Info className="text-[#0056b3] dark:text-blue-400 shrink-0" size={24} /> 
          Enflasyon ve Satın Alma Gücü Hesaplama Hakkında Her Şey ve Sıkça Sorulan Sorular
        </h3>
        ...
        <div className="prose prose-sm md:prose-base max-w-none text-slate-600 dark:text-slate-400 space-y-8 leading-relaxed">
            ...
            <div className="space-y-4 bg-slate-50 dark:bg-slate-800/30 p-4 md:p-6 rounded-2xl border border-black/5 dark:border-white/5">
              <div>
                ...
                <div className="bg-white dark:bg-slate-900 border border-black/10 dark:border-white/10 px-4 py-2.5 rounded-xl font-mono text-sm text-[#0056b3] dark:text-blue-400 mb-2 overflow-x-auto">
                  Gelecekteki Değer = Bugünkü Değer × (1 + (Yıllık Enflasyon Oranı / 100)) ^ Yıl
                </div>
              </div>
            </div>
          </div>
          ...
          <section className="pt-4 border-t border-black/5 dark:border-white/5">
            ...
            <div className="space-y-6">
              <div>
                <h5 className="font-bold text-slate-800 dark:text-slate-200 mb-2">Enflasyon ve Satın Alma Gücü Hesaplama aracı ücretli midir?</h5>
      ...
      <RelatedTools category="finans" currentToolId="enflasyon-hesaplama" />
      <div className="mt-8">
