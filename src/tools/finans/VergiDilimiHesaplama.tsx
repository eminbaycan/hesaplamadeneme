...
import { ChevronRight, RefreshCw, Copy, Info, Calculator, AlertCircle, Check } from 'lucide-react';
...
export default function VergiDilimiHesaplama() {
  const [kümülatifMatrah, setKümülatifMatrah] = useState<number | ''>('');
  const [aylikMatrah, setAylikMatrah] = useState<number | ''>('');
  const [sonuc, setSonuc] = useState<{
    mevcutDilim: number;
    tahminiKesinti: number;
    kalanTutar: number;
  } | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [copied, setCopied] = useState(false);

  // Örnek gelir vergisi dilimleri (2024/2025 tarifesi)
  // Dilimler: 110.000 (%15), 230.000 (%20), 870.000 (%27), 3.000.000 (%35), üstü (%40)
  const getVergiOrani = (matrah: number) => {
    if (matrah <= 110000) return 15;
    if (matrah <= 230000) return 20;
    if (matrah <= 870000) return 27;
    if (matrah <= 3000000) return 35;
    return 40;
  };

  const handleCalculate = () => {
    if (kümülatifMatrah === '' || aylikMatrah === '') {
      setError('*Lütfen kümülatif vergi matrahı ve aylık matrah alanlarını doldurun.');
      setSonuc(null);
      return;
    }
    setError(null);

    const kum = Number(kümülatifMatrah);
    const aylik = Number(aylikMatrah);

    const mevcutDilim = getVergiOrani(kum);
    const tahminiKesinti = aylik * (mevcutDilim / 100);
    const kalanTutar = aylik - tahminiKesinti;
  ...
  const handleCopy = () => {
    if (!sonuc) return;
    const text = `Vergi Dilimi: %${sonuc.mevcutDilim} | Kesinti: ${sonuc.tahminiKesinti.toFixed(2)} ₺`;
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      <div className="flex items-center gap-2 text-[13px] text-slate-500 dark:text-slate-400 mb-6 font-medium">
        ...
        <ChevronRight size={14} />
        <span className="text-slate-800 dark:text-slate-300">Gelir Vergisi Dilimi Hesaplama</span>
      </div>

      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-2">Gelir Vergisi Dilimi Hesaplama 2026 - Ücretsiz ve Hızlı Sonuçlar</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          Kümülatif gelir vergisi matrahınıza göre bulunduğunuz vergi dilimini ve kesinti tutarını öğrenin.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-4">
          <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-bold text-slate-800 dark:text-white">Matrah Bilgileri</h2>
              <button 
                onClick={handleClear}
            ...
            <div className="space-y-5">
              <div>
                <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">
                  Kümülatif Gelir Vergisi Matrahı (TL) <span className="text-red-500">*</span>
                </label>
                <input 
                  type="number" 
                  step="0.01"
                  value={kümülatifMatrah}
                  onChange={(e) => {
                    setKümülatifMatrah(e.target.value === '' ? '' : Number(e.target.value));
                    setError(null);
                  }}
              ...
              <div>
                <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wide">
                  Bu Ayki Vergi Matrahı (TL) <span className="text-red-500">*</span>
                </label>
                <input 
                  type="number" 
                  step="0.01"
                  value={aylikMatrah}
                  onChange={(e) => {
                    setAylikMatrah(e.target.value === '' ? '' : Number(e.target.value));
                    setError(null);
                  }}
              ...
              <div>
                ...
                {sonuc !== null ? (
                  <div className="space-y-4 mb-6">
                    <div>
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">Mevcut Vergi Dilimi Oranı</span>
                    ...
                    <div>
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">Bu Ayki Gelir Vergisi Kesintisi</span>
                      <div className="text-lg font-bold text-rose-600 dark:text-rose-400">
                        - {sonuc.tahminiKesinti.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺
                      </div>
                    </div>
                  </div>
                ) : (
                  <div className="py-12 text-center text-slate-400 text-sm">
                    Matrah bilgilerini girip hesapla butonuna tıklayın.
                  </div>
                )}
              </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm space-y-6">
        <h3 className="text-xl font-bold text-slate-800 dark:text-white flex items-center gap-2 border-b border-black/5 dark:border-white/10 pb-4">
          ...
          Gelir Vergisi Dilimleri Rehberi
        </h3>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          Gelir vergisi oranları, yıl içinde elde ettiğiniz kümülatif kazancın artmasıyla birlikte dilimler halinde %15'ten %40'a kadar kademeli olarak yükselir.
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 rounded-3xl p-6 md:p-8 shadow-sm mb-8">
        <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-6 flex items-center gap-2 border-b border-black/5 dark:border-white/10 pb-4">
          <Info className="text-[#0056b3] dark:text-blue-400 shrink-0" size={24} /> 
          Gelir Vergisi Dilimi Hesaplama Hakkında Her Şey ve Sıkça Sorulan Sorular
        </h3>
        ...
        <div className="prose prose-sm md:prose-base max-w-none text-slate-600 dark:text-slate-400 space-y-8 leading-relaxed">
            ...
            <div className="space-y-4 bg-slate-50 dark:bg-slate-800/30 p-4 md:p-6 rounded-2xl border border-black/5 dark:border-white/5">
              <div>
                ...
                <div className="bg-white dark:bg-slate-900 border border-black/10 dark:border-white/10 px-4 py-2.5 rounded-xl font-mono text-sm text-[#0056b3] dark:text-blue-400 mb-2 overflow-x-auto">
                  Ödenecek Gelir Vergisi = Kümülatif Vergi Matrahı × İçinde Bulunulan Dilimin Vergi Oranı (%) (Artan oranlı hesaplanır)
                </div>
              </div>
            </div>
          </div>
          ...
          <section className="pt-4 border-t border-black/5 dark:border-white/5">
            ...
            <div className="space-y-6">
              <div>
                <h5 className="font-bold text-slate-800 dark:text-slate-200 mb-2">Gelir Vergisi Dilimi Hesaplama aracı ücretli midir?</h5>
      ...
      <RelatedTools category="finans" currentToolId="vergi-dilimi-hesaplama" />
      <div className="mt-8">
